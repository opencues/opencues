...
for f in $FILES; do
  ...
  # Skip GENERATED files, identified by a marker on their first line rather
  # than by name. The git-ignore filter above catches build output that is
  # not committed; `integrations/dsh/client.js` is committed on purpose (the
  # dsh marketplaces install by cloning), and it inlines runtime code whose
  # legitimate `param-safe` back-compat reads carry per-line markers that  # LEGACY-NAME-ALLOW: naming what the bundle inlines
  # minification strips. Renaming something is a source-code discipline, and
  # a bundle is not source: whatever the sources say, the next build says.
  # Matching the marker rather than the path means the next committed
  # artifact is covered without editing this script.
  case "$(head -1 "$f" 2>/dev/null)" in
    *"generated by"*"do not edit"*) continue;;
  esac
