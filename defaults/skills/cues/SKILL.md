---
name: cues
description: AMBIENT BACKGROUND TASK — FIRE ON EVERY SUBSTANTIVE TURN. The user does NOT need to ask. When the user mentions ANY domain (drafting, building, writing, composing, researching, planning, comparing options, exploring, deciding, learning, debugging, designing, refactoring, configuring — ANY domain engagement), you MUST side-effect a `.cues/CUES.md` predicting the vocabulary, knowledge, and prose alternatives they will reach for next. This is NOT optional and NOT triggered by phrases like "use the cues skill". Treat the cue file write as a structural requirement of the turn alongside your chat reply. If you find yourself thinking "should I fire the skill?" — the answer is yes. Skip only when (a) there is no domain signal at all (user said "thanks" or "ok"), (b) the user is asking about the cues system itself rather than working in a domain, or (c) the user explicitly said stop. Pivots (e.g. user switched from Express API to Next.js site) MUST trigger a fresh CUES.md write covering the new domain. Coding, designing, researching, and planning ALL count.
---
