...
//      [--gate typesafe]  adds a GATED arm (Jev plan step 2): the source's exact pre-gate request
...
const decisions = GATE === 'typesafe' ? new core.TypeSafeDecisionProvider({ apiKey: process.env.TYPESAFE_API_KEY ?? '', httpAdapter: http }) : null;
if (GATE && !decisions) { console.error(`unknown --gate ${GATE}`); process.exit(2); }
if (decisions && !process.env.TYPESAFE_API_KEY) { console.error('TYPESAFE_API_KEY is required for --gate typesafe'); process.exit(2); }
