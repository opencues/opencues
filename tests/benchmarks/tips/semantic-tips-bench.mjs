...
//        [--provider typesafe] the DECISION matcher (decision layer): the real SemanticTipsSource on the installed decision package, one Choice over the pack — same cases, same gate, same usage line (priced from core's table)
...
const LEGS = DECISION ? core.loadDecisionLegs({ which: 'typesafe', apiKeys: process.env, httpAdapter: http, log: (m) => console.error(m) }) : null;
if (DECISION && !LEGS) { console.error('--provider typesafe needs the decision package (@opencues/decisions or OPENCUES_DECISIONS_PATH) and its key'); process.exit(2); }
if (DECISION && !argv.includes('--model')) MODEL_OVERRIDE = LEGS.model;
