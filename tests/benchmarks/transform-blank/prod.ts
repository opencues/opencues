...
import { loadDecisionLegs, type DecisionLegs } from '../../../packages/opencues-core/src/decisions/load';
 ...
 *  detector or the decision-layer one (the decision layer; needs
...
function buildSource(providerId: string, replaceParse?: string): TransformBlankSource {
  ...
  let decisions: DecisionLegs | undefined;
  if (replaceParse === 'decisions') {
    decisions = loadDecisionLegs({ which: 'typesafe', apiKeys: process.env, httpAdapter, log: (m) => console.error(m) });
    if (!decisions) { console.error('--replace-parse decisions needs the decision package (@opencues/decisions or OPENCUES_DECISIONS_PATH) and its key'); process.exit(1); }
  }
