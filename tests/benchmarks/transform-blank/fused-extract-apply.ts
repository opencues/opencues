/**
 * FUSED extract+apply — single LLM call that produces both the structured
 * decomposition (VERDICT / INSTRUCTION / TARGET) AND the rewritten TARGET.
 *
 * Premise: a "smarter" model (Gemini 3.1 flash-lite) can lay out the
 * structure and apply it in one pass, replacing two API hops with one.
 * Forcing the model to emit INSTRUCTION + TARGET as separate fields
 * before REWRITE acts as inline chain-of-thought scaffolding — cheaper
 * than two API calls, but more structured than a pure single-call.
 *
 * Output format — four lines (REWRITE may span many):
 *   VERDICT: TRANSFORM | NONE
 *   INSTRUCTION: <verb phrase, optionally pipe-joined for composed ops>
 *   TARGET: <rest of text>
 *   REWRITE: <rewritten TARGET; empty when NONE>
 */

import { chat, sysUser } from './groq';

const SYSTEM_PROMPT = `You read a sentence containing _ and decide whether it carries an IMPERATIVE INSTRUCTION the user wants applied to the surrounding text. If so, you both EXTRACT the instruction/target AND APPLY it in one shot.
...
Output exactly four labelled lines (REWRITE may span multiple lines):
VERDICT: TRANSFORM | NONE
...
REWRITE: ran to the store and bought milk
...
REWRITE: the children ran to the parks and found mice
...
REWRITE: The girl ran. The boy met another. They played.
...
export function parseFusedOutput(raw: string, latencyMs: number): FusedResult {
  // VERDICT and INSTRUCTION are single-line — use [ \t]* not \s* to avoid
  // swallowing the next label across a newline.
  const verdictMatch = raw.match(/^VERDICT:[ \t]*(TRANSFORM|NONE)[ \t]*$/im);
  const instructionMatch = raw.match(/^INSTRUCTION:[ \t]*(.*?)[ \t]*$/im);
  // TARGET can span multiple lines (sandwiched layout) — stop at the
  // next REWRITE: label.
  ...
  return { verdict, instruction, target, rewrite, raw, latencyMs };
}
