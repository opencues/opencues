...
**Non-standard.** `OPENCUES.md` is the OpenCues runtime's settings file. Its schema lives in [`../../opencues-runtime.md`](../../opencues-runtime.md), not in `core.md`. Other runtimes that park their config in a different file are conformant — this fixture is included so reference-implementation runners have a canonical example to parse.
