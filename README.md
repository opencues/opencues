...
- **Script-backed blanks** — `volume _` / `brightness _` call shell scripts so cycling actually changes your OS state. Type a value to set it directly (`volume 30 _`) or nudge it (`volume up _`). Routing is deterministic: a keyword fires only when it *leads the line* containing `_`, so `weather in london _` triggers a fetch but `the weather was lovely today _` (prose, keyword mid-line) never does.
