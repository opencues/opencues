...
- The same `blanks/volume/cue.md` (with its colocated `.sh` script) works on every native host that can spawn a subprocess. Chrome filters the script out via the `host-compat` declaration.
