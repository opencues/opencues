...
> **What works in Chrome vs native hosts**: cycling, blanks, step controls, opencues-settings (`opencues settings _`), prompt-improver, stocks/weather/HackerNews — all work. **What doesn't**: brightness / volume controls (no subprocess access in browsers — these spawn shell scripts on native hosts). TTS uses the Web Speech API (not `speak.sh`). Configs come from chrome.storage with bake-time fallback — no filesystem reads.
