...
| `focus` | `app`, `text`, `cursor`, `cycling`, `fieldId` | An **attachable** field gained focus (already passed the shim's editable + non-sensitive + not-deny-listed checks). Carries the field's current contents. `cycling` (phase 2): true when the field is UIA-attached with a managed TextPattern — the shim can hook chords + paint the overlay there; feeds the adapter's per-field `supportsCycling`. `fieldId` (phase 2): the shim's stable element-id hash, used for same-field resume. The daemon resets buffer state and seeds the mirror — UNLESS this is a same-field resume (same `fieldId` as the last blur AND unchanged text), in which case the runtime's spans survive and only the overlay repaints. |
| `blur` | `app` | Focus moved to something we don't touch (browser, terminal, password box, non-editable). Daemon detaches; the buffer-state reset is DEFERRED to the next `focus` (so click-away-and-back to the same field resumes with spans intact). While detached, inbound text/key events are ignored and outbound writes are dropped, so the preserved state is inert. |
