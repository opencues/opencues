...
2. **`integrations/claude-code/patches/setup.sh`** (CC-specific only)
   - Default behavior: nuke + rebuild from scratch (`~/.opencues/forks/claude-code/{node_modules/@anthropic-ai, node_modules/@opencues, .cues/}`). `--keep-state` flag skips the nuke for dev iteration.
