...
export function dispatchOpenCuesKey(evt: any): boolean {
  ...
  // with no Ctrl byte). Every other terminal — Ghostty, iTerm2, Linux xterm,
  // Windows Terminal — uses xterm-modifier-encoded CSI (`\x1b[1;7A`) with the
  // Ctrl bit present, so the synth is a no-op there. We gate on the raw byte
  // sequence rather than on `evt.option` because OpenTUI ALSO sets
  // `option: true` for `\x1b[1;3A` (plain Alt+arrow on Linux — modifier byte
  // bit 2). Gating on the literal `\x1b\x1b[` prefix means the synth can never
  // hijack plain Alt+arrow on Linux/Windows.
