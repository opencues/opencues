 ...
 *    LLM POSTs through a `browser.runtime.sendMessage` proxy here saves
...
//
// Keyed off the same browser.storage entry the bootstrap reads
...
async function refreshDebugMode(): Promise<void> {
  try {
    const result = await browser.storage.local.get(DEBUG_MODE_KEY);
...
browser.runtime.onMessage.addListener((message: FetchRequest, _sender, sendResponse) => {
...
// every filesystem change. We persist the bundle to browser.storage.local
  ...
  nativePort.onMessage.addListener((raw: unknown) => {
    ...
    if (msg.type === 'bundle' && typeof msg.files === 'object') {
      ...
      void isDeferEnabled().then((enabled) => {
        ...
        return browser.storage.local.set({ [BUNDLE_KEY]: { files: msg.files, root: msg.root } })
    ...
    if (msg.type === 'config' && msg.apiKeys && typeof msg.apiKeys === 'object') {
      ...
      void isDeferEnabled().then((enabled) => {
        ...
        return browser.storage.local.set({ [HOST_KEYS_STORAGE]: apiKeys })
...
browser.runtime.onMessage.addListener((message: ExecRequestFromContent, _sender, sendResponse) => {
...
browser.runtime.onMessage.addListener((message: UserBlankInvokeRequestFromContent, _sender, sendResponse) => {
...
browser.runtime.onMessage.addListener((message: { type?: string; level?: string; msg?: string; data?: unknown }, _sender, sendResponse) => {
...
// shape signals that so the caller can fall back to browser.storage —
...
browser.runtime.onMessage.addListener((message: WriteFileRequestFromContent, _sender, sendResponse) => {
