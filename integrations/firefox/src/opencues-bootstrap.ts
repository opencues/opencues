...
//   - readFile/writeFile route through browser.storage.local using
...
// and live cycling — browser.storage.onChanged subscription updates
...
function mirrorToHostLog(level: 'info' | 'debug' | 'warn' | 'error', args: unknown[]): void {
  ...
  try {
    browser.runtime.sendMessage({ type: 'opencues:log', level, msg, data: safeData })
      .catch(() => { /* no listener */ });
...
browser.storage.onChanged.addListener((changes, area) => {
  ...
  //
  // ALSO converge browser.storage's OPENCUES.md per-key entry to the
  ...
  // that only knew about browser.storage) would mask the bundle's
  ...
  if ('opencues_bundle' in changes) {
    ...
    if (typeof bundledOpencuesMd === 'string') {
      const key = `${STORAGE_PREFIX}${ROOT}/.cues/OPENCUES.md`;
      void browser.storage.local.set({ [key]: bundledOpencuesMd }).catch(() => { /* swallow */ });
    }
    if (bootResult) {
...
/** Real-time API-key update — called by content.ts when browser.storage
 ...
 *      a. browser.storage.local['opencues_bundle']  ← native-messaging host
 *      b. browser.runtime.getURL('dist/configs/<rel>')  ← bake-time bundle
 ...
 *   2. browser.storage.local[STORAGE_PREFIX + path]  ← user saved state
...
async function readFile(path: string): Promise<string | null> {
  ...
  //    prompt changed but browser.storage still held the old seeded
...
//   1. browser.storage.local['opencues_bundle']  ← pushed by native host
//      (`opencues install chrome-host`). Contains files: { [rel]: content }.
//   2. browser.runtime.getURL('dist/configs/...')  ← bake-time bundle
...
async function registerUserBlanksFromBundle(
  ...
  const stored = await browser.storage.local.get(['opencues_bundle']);
...
function getBundleIndex(): Promise<{ files: Set<string>; storage: StoredBundle | null; loaded: boolean }> {
  ...
  _bundleIndexPromise = (async () => {
    ...
    // 1. Native-messaging push lives in browser.storage.local. If
    ...
    try {
      const stored = await browser.storage.local.get(BUNDLE_STORAGE_KEY);
    ...
    try {
      const url = browser.runtime.getURL('dist/configs/index.json');
...
// bundle into browser.storage, that wins; otherwise we fetch from the
...
async function readBundledConfig(runtimePath: string): Promise<string | null> {
  ...
  try {
    const url = browser.runtime.getURL(`dist/configs/${rel}`);
    const res = await fetch(url);
 ...
 *  chrome-host`), we fall back to writing browser.storage — the legacy
  ...
  if (path === ROOT + '/.cues/OPENCUES.md' || path === ROOT + '/.cues/NOTES.md') {
    try {
      ...
      if (reply?.ok) {
        ...
        const key = STORAGE_PREFIX + path;
        try { await browser.storage.local.set({ [key]: content }); } catch { /* */ }
        return;
      }
      // Host disconnected → fall through to storage-only.
      log.info(`[opencues] writeFile(${path}): host unavailable (${reply?.error ?? 'no reply'}), falling back to browser.storage`);
    } catch (err) {
      log.info(`[opencues] writeFile(${path}): sendMessage threw, falling back to browser.storage: ${String(err)}`);
    }
  }
  const key = STORAGE_PREFIX + path;
  try {
 ...
 * flow through browser.storage only when the runtime actually writes
...
export interface RuntimeStartOptions {
  ...
  /**
   * Multi-provider key bag. The popup writes these to browser.storage
 ...
 *
 *  Reads from browser.storage's merged OPENCUES.md key. Falls back to
...
async function auditProvidersAgainstKeys(keys: Record<string, string>): Promise<void> {
  ...
  // keys-available state than chrome (which reads browser.storage).
  ...
  log.warn(
    ...
    `(chrome reads keys from browser.storage — env vars do NOT propagate):\n` +
    problems.join('\n') + '\n' +
  ...
  await Promise.all(Object.entries(keys).map(async ([provider, key]) => {
    ...
    try {
      const reply = await (browser.runtime.sendMessage({
        ...
        url,
        headers,
      }) as Promise<{ ok: boolean; status: number; statusText: string; text: string }>);
      if (reply?.ok) {
...
export function startOpenCues(opts: RuntimeStartOptions = {}): BootResult {
  ...
  const blanks = createBlanks({
    ...
    // reads/writes the seeded OPENCUES.md in browser.storage.
    ...
    // chokepoint BEFORE this writer is called. browser.storage-backed:
  ...
  // through browser.storage only when they're actually written.
  bootResult = boot({
    hostVersion: '0.1.0',
    cwd: ROOT,
    ...
    // to browser.runtime.connectNative directly (SW-only API), so we
    // proxy via browser.runtime.sendMessage → SW forwards over the
  ...
  // picked up by the browser.storage.onChanged listener registered
  ...
  // browser.storage.local['opencues_bundle'], and the storage-onChanged
  ...
  const loadCalendarContext = async (): Promise<void> => {
    try {
      ...
      if (!raw) {
        try {
          const r = await fetch(browser.runtime.getURL('dist/configs/calendar.json'));
  ...
  browser.storage.onChanged.addListener((changes, area) => { if (area === 'local' && changes['opencues_bundle']) void loadCalendarContext(); });
  ...
  // Eager bundle-to-storage sync at boot. browser.storage.onChanged
