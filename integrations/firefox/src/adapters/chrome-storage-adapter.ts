...
async function readKeyBags(): Promise<KeyBags> {
  try {
    const result = await browser.storage.local.get([HOST_KEYS_STORAGE, USER_KEYS_STORAGE]);
...
async function migrateStaleConfig(stored: Record<string, unknown> | undefined): Promise<void> {
  ...
  if (hasStaleApiKey) {
    const existing = await browser.storage.local.get(USER_KEYS_STORAGE);
    ...
    if (!userKeys.GROQ_API_KEY) {
      ...
      await browser.storage.local.set({ [USER_KEYS_STORAGE]: userKeys });
    }
...
export async function loadConfig(): Promise<StoredConfig> {
  ...
  // that promoted value.
  const storedRaw = await browser.storage.local.get(STORAGE_KEY)
  ...
  const [stored, bags] = await Promise.all([
    browser.storage.local.get(STORAGE_KEY).then(r => r[STORAGE_KEY] as Record<string, unknown> | undefined),
    readKeyBags(),
  ]);
...
export async function saveUserKeys(keys: Record<string, string>): Promise<void> {
  ...
  await browser.storage.local.set({ [USER_KEYS_STORAGE]: current });
}
...
export async function resetConfig(): Promise<void> {
  await browser.storage.local.remove([STORAGE_KEY, USER_KEYS_STORAGE]);
}
