...
async function init(): Promise<void> {
  ...
  void browser.runtime.sendMessage({ type: 'opencues:host-status' }).then((reply: unknown) => {
  ...
  // Live-save on change — fires browser.storage.onChanged, which
...
async function runDiagnostic(): Promise<void> {
  ...
  try {
    ...
    log(`✗ browser.tabs.query failed: ${(err as Error).message}`);
    return;
  }
  if (!tab?.id) { log('✗ no active tab'); return; }
  log(`● active tab: ${tab.url?.slice(0, 60) ?? '(no url)'}`);

  const restricted = !tab.url || /^(chrome|chrome-extension|moz-extension|edge|about|view-source|resource):/.test(tab.url);
  if (restricted) {
    log('✗ active tab is a restricted URL — content scripts can NOT inject on about:, moz-extension://, etc. Test on a real https:// page (try wikipedia.org).');
    return;
  }
  ...
  try {
    const response: unknown = await Promise.race([
      browser.tabs.sendMessage(tab.id, { type: 'opencues:diagnostic-ping' }),
  ...
  try {
    const storage = await browser.storage.local.get(['opencues_user_keys', 'opencues_host_keys', 'opencues_bundle']);
