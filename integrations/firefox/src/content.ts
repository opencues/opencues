...
//   1. Load popup config from browser.storage.
...
async function init(): Promise<void> {
  ...
  // Boot the runtime with config from browser.storage. The runtime
  ...
  // browser.storage on its own; we just need to re-publish the target
...
browser.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  ...
  void loadConfig().then(liveConfig => {
    ...
    sendResponse({
      bootVersion: browser.runtime.getManifest().version,
