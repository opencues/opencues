...
const blanksRegistry: Map<string, Blank> = createDefaultBlanksRegistry({
  ...
  // IDENTITY.md sibling — the keyword-bound `set sentinel _` /
...
export function _discoverUserBlankConfigs(): BlankConfigLike[] {
  ...
  // and every user-blank registration fires a "name collision"
  // warning.
...
const _userBlanks = buildUserBlankRegistry(_discoverUserBlankConfigs(), {
  ...
  // Debug-level chatter gated behind DEBUG_OPENCUES=1.
  log: (lvl, msg) => {
...
export function triggerOpenCuesRender(text: string, cursor: number): void {
  ...
  if (ocStyleIdsCache.highlight === undefined) {
    ...
    // (RGBA) natively. No bold — the white-on-black contrast is
    // already strong enough that bold becomes visual noise.
