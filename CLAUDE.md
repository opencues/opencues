...
- **docs/artifacts/** — the OpenCues artifact kit: theme + build script + hero animation for building OpenCues-branded HTML pages (the design-review / feature-reference pages we publish as artifacts). `README.md` for the quickstart, `REFERENCE.md` for tokens, components, the hero engine, and the two mobile gotchas (`white-space:pre` inter-element whitespace; `min-width:0` on grid children). An override applied deliberately, not a default.
