...
- **Claude Code** (`integrations/claude-code/`) — patches Claude Code 2.1.x (current pin 2.1.170 native binary; tested versions in `integrations/claude-code/compat.json`) via tweakcc **pinned to `v4.3.0`** (`compat.json:tweakcc-pin` — NOT HEAD; 4.3.1 regressed the native-binary `.bun` repack)
...
| Unpinned tweakcc clone → a bad tweakcc release corrupts the CC native binary | July 2026 — `setup.sh` cloned `github.com/Piebald-AI/tweakcc` at **HEAD**, which pulled the day-old **4.3.1**. 4.3.1 regressed the native-binary `.bun`-section repack: the repacked `claude.exe` bloats 247MB→411MB and crashes at startup with `TypeError: Expected CommonJS module to have a function wrapper` (Bun can't load the corrupted module graph). OpenCues never boots, no `[cc]` log lines, the harness bridge never arms — but `validateFork` reported the fork HEALTHY because its boot-smoke only `require()`s the dist modules under Node; it never ran the actual Bun binary. A month-old working fork (core 0.13.7) was silently replaced with a dead binary on reinstall. | (1) `setup.sh` now clones then `git checkout`s an EXACT tag from `compat.json:tweakcc-pin` (`v4.3.0`), not HEAD; env `TWEAKCC_REF=<tag>` overrides for bisecting a new release. (2) `install.cjs:validateFork` gained a native-binary startup smoke — `spawnSync(claude.exe, ['--version'])` must exit 0 and print `Claude Code`, so a corrupt repack can never ship as "healthy" again. Bump the pin only after a newer tag `--version`s cleanly. | `validateFork` binary-startup smoke (native shape); `setup.sh` aborts if the pinned tag doesn't exist |
