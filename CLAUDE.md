...
- **TWO pins move together on every CC bump, both in `integrations/claude-code/compat.json`:** `current-pin` (the `@anthropic-ai/claude-code` version) and `tweakcc-pin` (the Piebald-AI/tweakcc commit `setup.sh` checks out after clone — normally upstream's `Prompts for <version>` commit). tweakcc is both the patch engine and a per-CC-version prompt-regex catalogue, so validating one against a stale other is not a validation. A CC-bump PR that touches `current-pin` without `tweakcc-pin` (or vice versa) is incomplete — the UPGRADING runbook's step 3 lists both.
- **If the patched binary dies at PARSE time (`SyntaxError ... ternary operator`) with all seams green**: that's tweakcc's system-prompt writeback corrupting nested-template prompts — disabled by `setup.sh` § 4e since the 2.1.206 bump. Diagnose with `bun build --no-bundle` against `<fork>/.cues/patch-state/native-claudejs-patched.js` (NOT `node --check` — cli.js uses the `using` keyword, which Node ≤22 can't parse). Post-mortem: `packages/opencues-runtime/adapters/cc/REPAIR.md` § 15.
