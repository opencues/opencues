...
- **docs/artifacts/** — the OpenCues artifact kit: theme + build script + hero animation for building OpenCues-branded HTML pages (the design-review / feature-reference pages we publish as artifacts). `README.md` for the quickstart, `REFERENCE.md` for the full rule set — tokens, the site's spacer scale, components, the hero engine, and seven gotchas (`white-space:pre` inter-element whitespace; `min-width:0` on grid children; `transform` silently ignored on inline elements; centred layout turning a height change into visible motion; measurement traps; and the rule that everything depicting the user's text borrows the terminal's colours). `PIPELINE.md` for artifact → website. An override applied deliberately, not a default.
