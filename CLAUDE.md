...
│   ├── CUES.md                    # Cue master: project metadata frontmatter + ## Ignore / ## Prompt sections (LLM cue sources); legacy ## Tips block feeds the semantic-tips catalogue
...
│   ├── cues/                      # Folder-based cue configs (LLM word-cues + per-host tips packs) — 8 shipped folders
...
- **docs/architecture/decisions.md** ⚠️ Canonical reference for the DECISION seam (`packages/opencues-core/src/decisions/`): a calibrated decision model answers typed `noul` / `choice` / `score` questions with probabilities and never generates text, so it is a separate seam from the LLM providers (never in `PROVIDERS`, never a fourth bucket) behind ONE scalar, `decisions-provider` (default off). The sources never see a question: they hand a LEG (`DecisionLegs` in `decisions/legs.ts` — `pause`, `sentenceGate`, `route`, `replace`, …) the thing to judge and get back a verdict over inputs the runtime supplied. The package that answers is loaded BY NAME (`decisions/load.ts`: `OPENCUES_DECISIONS_PATH`, then `@opencues/decisions`); without one every leg keeps its chat path, with one log line. `dispatchDecision` is the single chokepoint (validate → PII floor over every string in the state → ask → usage meter → `[decision][leg]` log line); `DecisionBreaker` is shared by the rail and the `_` router. Every consumer keeps its chat path and falls through on a failed, slow or absent leg. No visual change at any leg: confidence is carried as data and left unrendered. The TypeSafe provider adapter (`decisions/typesafe.ts`) and what a leg OFFERS the model (`decisions/candidates.ts`) are public; the reference package (question templates, thresholds inside a leg, the fused composition, benches, numbers) is PRIVATE (`opencues/decisions`); never move a question template, a leg threshold, or a bench number into this repo. Read before adding a leg or touching the dispatch.
...
- **docs/architecture/contradiction-cues.md** ⚠️ Canonical reference for `contradiction-cues-mode` — the deterministic fact-check cue engine (`packages/opencues-core/src/contradiction/`). Parse (one cues-bucket LLM call turns a sentence into a TYPED claim) → verify (the runtime computes the correction: real weekday, arithmetic, world-data cache read) — the correction is DATA, never generation, so a cue can't hallucinate a false contradiction. ONE scalar (off by default); tiers are DATA-GATED, not per-scalar: Tier 0 (weekday-date, split-the-bill) needs only buffer + clock; 0.5 (bank holidays / GOV.UK), 5 (outdoor-plan vs weather / open-meteo), 5b (London transit / TfL), 5c (journey-underestimate / photon geocode), 5d (subreddit rules / same-origin `about/rules.json`, chrome-only via the `pageLocation` getter; the ONE tier whose conflict judgement is a dedicated LLM call — tip text still comes from the cached rule data) each activate only when their `{refresh,current}` cache is host-wired (absent → that verifier is a no-op). Renders as a PASSIVE sentence-cue (priority 87; never auto-splices). Egress hosts hardcoded (never LLM-chosen); journey inputs grounded via `isGeocodableName` (security-audit #28–#29). Read before touching `checks.ts` / `contradiction-llm-source.ts` / the per-provider `journey.ts`/`tfl.ts`/`weather.ts` or the resolver's `enableContradictionCues` + `worldDataFetch` wiring. User-facing summary: `docs/features/contradiction-cues.md`.
- **docs/architecture/cue-dismissal.md** ⚠️ Canonical reference for dismissing a cue — the `_` gesture on an advisory note. TWO GRAINS: one press mutes for 30 min (in memory, `state/cue-dismissals.ts`, lapses on its own, nothing written); a second press within a 3s OFFER WINDOW forgets it (written to user-level `<cues>/dismissals.json`, listed + reversible via **`opencues dismissals`**). ⚠ The offer window is load-bearing: a mute stops the note painting, so without it there is nothing left on screen to press `_` on again and the forget grain is unreachable — for `FORGET_GRACE_MS` the note keeps painting with the hint `(muted · underscore again to forget)`, and Cycling claims `_` only while it is visible. Confined to a PURE ADVISORY (a `cueTip` def with `alternatives.length <= 1` — calendar clash, advisory contradiction verdict), so `_` still means "cycle" wherever there are alternatives to walk; the note's hint switches between `(underscore to cycle)` and `(underscore to dismiss)` accordingly. Enforced at three sites, each covering what the others can't: the gesture (`Cycling.stepUnderscore`, BEFORE the cycleable guard so the `_` is consumed, never typed), the paint (`DimRender`, covers defs registered pre-dismissal), and registration (`Resolver`, which also keeps it off the secondary display). **⚠ THE KEY IS THE NORMALIZED TEXT, NEVER A COMMITMENT ID** — `c<N>` ids are per-snapshot ordinals (`buildSessionCommitmentsSnapshot` assigns positionally on every write; `mergeSessionCommitments` puts fresh first, renumbering the rest), verified on real snapshots where one decision is `c1` in one watchlist and `c5` in another; keying on an id would silence whichever cue landed in that slot next tick. `dismissalKey` is pinned by test to `normalizeCommitmentStatement` so dismissal and supersession share one notion of "the same claim"; documented limit is that a rephrasing reads as a new cue. `startCueDismissals` (wired from `buildSharedRuntime` for the six bands that call it, and BY HAND in the CC band, which predates that helper — pinned by `boot-bands-wiring.test.ts`, which matches on the call with comments stripped) hydrates at boot + re-reads on a 4s mtime gate, which is what makes the CLI restore land in a live session with no restart. Chrome has no filesystem → no sink → forget degrades to a 24h mute rather than claiming a permanence a restart would expose. No scalar (a gesture, not a mode). User-facing summary: `docs/features/cue-dismissal.md`.
- **docs/architecture/semantic-tips.md** ⚠️ Canonical reference for the tips system since spec 0.12: the packs are SITUATIONS (`when:` + `say:` per entry, no sibling lists), matched against the whole draft by one fast-model call per pause (`SemanticTipsSource`, priority 86, the tips leg of the fused session rail) with three grounding rules (cited id exists, quote is verbatim, the solution is the entry's own slash command or a rewrite that keeps the person's words), rendered as a 💡 passive note whose `_` swaps the draft for the command. `tips-mode: semantic | off`; `tips-shard-size` shards a large catalogue into parallel calls. There is NO static layer: a typed pack word is a plain word (no gray, no nav stop, no cycling). Bench: `tests/benchmarks/tips/semantic-tips-bench.mjs --pack <host>` (the gate; met on all four packs on gpt-oss and qwen); `--probe "<phrase>"` before promising any phrasing in a test list. Read before touching `tips-catalog.ts`, `semantic-tips-source.ts`, the tips leg of `SessionCueSource`, the resolver's sentence-cue registration, or a pack's `when:` / `say:` lines. User-facing: `docs/features/semantic-tips.md`.
- **docs/architecture/session-contradiction.md** ⚠️ Canonical reference for `session-contradiction-mode` — the "you're contradicting a decision you made earlier this session" cue, on any host with a session transcript (**Claude Code, OpenCode, Gemini CLI**; inert on shell/chrome). A DIFFERENT engine from `contradiction-cues-mode` (which is deterministic — model routes, runtime computes): this one is a two-stage WATCHLIST matcher. **Stage A (slow producer):** `opencues extract-commitments <transcript_path> [--format cc|gemini|opencode] [--cwd <dir>]` (`packages/opencues-cli/src/commands/extract-commitments.cjs`), kicked from EVERY host's boot band by a `startSessionCommitmentsKick` poller (`boot-common.ts`) over the host's own transcript (per-host locators: `locateNewestCCTranscript` for CC's `~/.claude/projects/<slug>/*.jsonl`, `locateNewestGeminiChat`, `locateOpenCodeDb`) — so the feature works out of the box, no opt-in statusline needed. CC ALSO kicks from the statusline `_oc_kick_commitments` (belt-and-braces; harmless double-kick — the producer self-debounces + locks). Distils the session transcript into a terse commitments watchlist. **Incremental distillation** (core ≥0.42.4): the watchlist ACCUMULATES across ticks (merge, not overwrite) so early decisions don't age out of the 256KB tail — preservation/dedup deterministic (`mergeSessionCommitments`), supersession its own small LLM call (`SESSION_COMMITMENTS_SUPERSEDE_SYSTEM`); per-session (resets on a new transcript). **Per-cwd scoped** output: `<cues>/session-commitments/<sessionCommitmentsKey(cwd)>.json` (flat `session-commitments.json` fallback when no `--cwd`). Only user+assistant PROSE reaches the producer (tool I/O + thinking dropped in the per-host parsers — the data-minimization boundary; security-audit #31). Routes to Claude Haiku when `ANTHROPIC_API_KEY` set (but the real-transcript bench favours cerebras/gemma — see `tests/benchmarks/session-contradiction/RESULTS-real-transcripts.txt`). **Stage B (fast matcher):** `SessionContradictionSource` (`packages/opencues-core/src/contradiction/session-contradiction-source.ts`) runs ONE debounced whole-buffer LLM call against the watchlist (in the SYSTEM msg → cerebras prefix-cache), emits a passive `sentence-cue:session-contradiction` result (priority 88). Fused with tips + ask-cues in `SessionCueSource` (contradiction and tips run in parallel since core 0.60.4, contradiction wins; ask only when neither flagged). Two grounding invariants: the flagged quote must be an exact buffer substring AND the cited commitmentId must be on the watchlist (else dropped). **Company/project rules (core ≥0.51.0):** a static `RULES.md` in the `.cues` search paths (project beats user; `- ` bullets are rules, prose ignored) merges into the same watchlist at the ingest — rules first with stable `r<N>` ids, session entries deduped against them (`commitmentDedupeKey`), capped at 24; benched 19/19 recall / 0 false alarms across five industries with the matcher prompt UNCHANGED (`tests/benchmarks/session-contradiction/company-rules-bench.mjs`). Advisory, never blocking; native hosts + dsh (its node half merges the same scopes via the same core fns, project = the SESSION's cwd; chrome deliberately not bridged). Catalog flows via `CueContext.sessionCommitments`: `buildSessionCommitmentsIngest(log, {cwd})` (`boot-common.ts`) reads the scoped snapshot on a timer into a live holder, every boot band passes it as `ResolverOptions.sessionCommitments`, resolver forwards it gated by the scalar. Render rail reused (passive, never auto-splices; `Ctrl+Alt+↑` applies the reconciled rewrite; `inline-cues-mode` governs inline vs statusline). **ON by default since core 0.48.0** — which retires the opt-in consent gate security-audit #31 used to lean on, so the prose-only parse boundary is now the whole of that defence (`ask-cues-mode` went on at the same time but was reverted to opt-in in 0.50.1: ~1-in-3 useful on realistic drafts, ceiling confirmed across every inference-time architecture — `tests/benchmarks/ask-cues/EXPERIMENTS.md`); runtime-only knob (NOT a spec change). Adapter-band boot edit + CC statusline bake are OUTSIDE srcHash — must run `setup.sh`/`opencues install` to deploy. User-facing summary: `docs/features/session-contradiction.md`.
...
- **docs/architecture/fluid-config.md** ⚠️ Canonical reference for the optional `fluid-config-mode` feature — semantic `_` → settings change classifier at priority 94 (between BlankSource 95 and TransformBlank 93). Routes ONLY to settings-registry scalars — FEATURES plus, since Sep 2026 (core 0.58.0), MENU_TUNABLES at their closed preset lists (never user blanks — volume / brightness / weather / etc. would widen the prompt-injection blast radius unacceptably; never a free number). Three structural defences: prompt enumerates only registry-cyclable values; `validateAgainstRegistry` runtime check rejects unknown setting / unlisted value / `exposeInMenu: false` (footgun-mode); apply path uses the same `applyOpenCuesScalar` (write + 2.5s reload-suppression) the satellite cycling has used since v0.1. Emits a selector-satellite-shaped result that wipes the summon phrase via `spanStart=summonPhraseStart(text)/spanEnd=text.length` (the last sentence terminator / line break before `_`, or 0 if none) — so prior user content before the settings command is preserved (`hii world. voice mode off _` keeps "hii world.") — and hands off to standard `cycleSelectorSatellite`; `clearOnEdit: true` lets backspace wipe the resulting pair as one span. Bench: `tests/benchmarks/fluid-config/` validated v2.1 prompt across 5 providers at 100% precision + 90-100% holdout recall. **Re-run that bench before editing `SYSTEM_PROMPT` in `config-intent-source.ts`.** User-facing summary: `docs/features/fluid-config.md`. Adding a feature to FEATURES or a tunable to MENU_TUNABLES automatically extends the classifier's choice space — no prompt edit needed. Model-emitted provider aliases (`claude` → anthropic, `google` → gemini) are normalised in `parseConfigIntentOutput` before validation.
...
| `packages/opencues-core/` | `@opencues/core` | 0.71.0 | private |
| `packages/opencues-runtime/` | `@opencues/runtime` | 0.47.1 | private |
| `packages/opencues-cli/` | `opencues` (real CLI) | 0.7.19 | **PUBLISHED on npm** (0.7.15) |
| `integrations/claude-code/` | `@opencues/claude-code` | 0.2.13 | private |
| `integrations/opencode/` | `@opencues/opencode` | 0.2.19 | private |
| `integrations/chrome/` | `@opencues/chrome` | 0.2.209 | private |
| `integrations/gemini-cli/` | `@opencues/gemini-cli` | 0.2.12 | private |
| `integrations/shell/` | `@opencues/shell` | 0.2.25 | private |
...
| A shared boot service wired only inside `buildSharedRuntime` is silently absent on Claude Code | Aug 2026 cue dismissals — `startCueDismissals` was wired once in `buildSharedRuntime` on the belief every band calls it. Six do; **`adapters/cc/v2.1/boot.ts` does not** (it predates the helper and hand-wires its modules, mentioning `buildSharedRuntime` only in comments). A `grep -l buildSharedRuntime` matched those comments, so the wiring looked universal. On CC the `_`-twice gesture fired and logged `cue forget`, then degraded to an in-memory mute because no sink was registered — nothing reached `<cues>/dismissals.json`, `opencues dismissals` stayed empty, and there was no error anywhere. Same shape as the PR #117 `checkRuntimeDrift` gap in the row below. | CC calls the service directly next to its other ingests; `packages/opencues-runtime/src/boot-bands-wiring.test.ts` walks every band's `boot.ts`, STRIPS COMMENTS, and asserts each one either calls `buildSharedRuntime` or wires the service itself — assert on the CALL, never the mention. | `boot-bands-wiring.test.ts` (runtime suite) |
