...
- **docs/architecture/blank-intent.md** ⚠️ Canonical reference for the optional `blank-intent-mode` feature (OFF by default; shipped v2026.06.25, core 0.5.1 / runtime 0.4.4) — an LLM invocation gate for keyword script-blanks. Two-phase: Phase-1 deterministic keyword pre-gate, Phase-2 LLM classifier (`BlankIntentClassifier` in `packages/opencues-core/src/sources/blank-intent-source.ts`) returning `{verdict: INVOKE|CEDE, action: get|set|step, value}`. Keyword-required Tier B (the LLM only REFINES an invocation the user signalled by typing the keyword; never summons a fetch/exec). **Typed actions**: `volume 30 _` sets, `volume up _` steps by `blankStep`; threaded into `BlankFill.maybeRunScripts`. The gate is built by `buildBlankIntentClassifier` (boot-common) AND wired inline in the CC band `adapters/cc/v2.1/boot.ts` (it builds `BlankFill` itself). **5-SITE KEYWORD-WINDOW INVARIANT (load-bearing)**: when the gate is on the keyword window is LINE-SCOPED (not per-blank `blankProximity`) via the single shared predicate `packages/opencues-core/src/keyword-window.ts` (`keywordInWindow` / `lineOfWords`), called by ALL FIVE sites that decide a keyword-blank claim — `BlankFill.matchKeyword` + `BlankSource` claim + `FluidBlank`/`TransformBlank`/`ConfigIntent` cede checks. **Change the window logic in one of those five and it MUST stay routed through `keywordInWindow`, or two sources fire on one `_` (the dev-time race: BlankFill set `brightness 90` while FluidBlank answered "percent").** A live `blankIntentLineScoped` getter is threaded resolver → build-sources → each source. Read before touching any keyword-claim/cede logic, `blank-fill.ts`'s gate, `keyword-window.ts`, or the catalog. Bench drives the BARE source (catalog-induced bugs only surface in the agentic host — scenarios 200–213). User-facing summary: `docs/features/blank-intent.md`.
