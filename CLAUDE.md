...
- **docs/architecture/decisions.md** ⚠️ Canonical reference for the DECISION provider seam (`packages/opencues-core/src/decisions/`): TypeSafe's Jev answers typed `noul` / `choice` / `score` questions with calibrated probabilities and never generates text, so it is a separate seam from the LLM providers (never in `PROVIDERS`, never a fourth bucket) behind ONE scalar, `decisions-provider: off | typesafe` (default off). `dispatchDecision` is the single chokepoint (validate → PII floor over every string in the state → ask → usage meter under `typesafe` → `[decision][leg]` log line); `validateDecisionRequest` rejects array-index path references (`` `rules[3]` `` is read one-off on the wire — key state by name). Model PINNED to `jev-1.13.0`, bench on bump. The doc carries the three primitives, the five mechanics with measured numbers (per-pause fan-out, candidate selection, per-unit fan-outs, probabilities as the product, whole-document passes), the limits (state ≤ ~32.9k tokens, total ≤ ~65.8k, no cache, no rate limiting seen to ~100 req/s, +344 ms cold), the authoring rules, and the eight-step integration plan. Every step reports one row from `tests/benchmarks/decisions/compare.mjs <leg>` (accuracy · p50/p90 · $/call · tokens, Cerebras arm vs Jev arm, same session) into `RESULTS.md`, and updates the running gains ledger `tests/benchmarks/decisions/GAINS.md` (per-call and per-event before/after, cumulative) in the same PR; a step ships only at accuracy parity with the cost or latency column lower. No visual change at any step: confidence is carried as data on results and left unrendered. Read before adding a leg, writing a question, or touching the dispatch.
...
| `packages/opencues-core/` | `@opencues/core` | 0.69.1 | private |
| `packages/opencues-runtime/` | `@opencues/runtime` | 0.41.9 | private |
| `packages/opencues-cli/` | `opencues` (real CLI) | 0.7.16 | **PUBLISHED on npm** (0.7.15) |
