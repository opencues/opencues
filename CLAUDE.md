...
- **mac** (`integrations/mac/`) — macOS-only universal host: answers `_` blanks in the **focused text element of ANY app** via the Accessibility API. A persistent Swift bridge (`ax-bridge.swift`) pushes focus/per-keystroke-change/cursor events (AXObserver — no polling) and applies ~1ms in-place range writes (atomic selection-free `AXReplaceRangeWithText` where the element honours it — its return value LIES, verified by re-read — else a select→replace→restore transaction). **Self-owned host**, universal/no-cycling profile; shares the `adapters/universal/v1` band with apple-notes (declares `hostName: 'mac'` for `on-host:` routing). The buffer IS the element's value, the cursor IS the real caret; host state is pure pieces in `src/ax-host.ts` plus the FULL bridge-event grammar in `src/daemon-core.ts` — both **platform-free and tested on any OS** (`daemon-core.test.ts` replays real bridge transcripts; the maintainer has no mac, so the suite must never grow a darwin/swiftc/AX dependency — `swiftc` is a darwin-gated build step via `scripts/build-bridge.cjs`, loud no-op elsewhere). Requires the Accessibility TCC grant (distinct from Automation), probed at install. Secure fields never leave the bridge; terminals denied by default. **Panel agents** (Spotlight; extend via `OPENCUES_AX_PANEL_AGENTS`) get persistent secondary observers — their non-activating LSUIElement panels never fire app activations, so the frontmost-app trigger is structurally blind to them; dismissal falls back to the primary observer via `AXApplicationHidden`. Channel evidence: `integrations/mac/AX-SPIKE.md` (general) + `integrations/mac/SPOTLIGHT-SPIKE.md` (panel agents). **Spotlight-only field semantics** (both keyed by bundle in `src/ax-host.ts`, both host-computed with no scalar gate): answers aim short (37 chars — `charBudgetForBundle` / `OPENCUES_AX_CHAR_BUDGET`, `docs/features/answer-char-budget.md`) AND **replace** the typed query instead of trailing after it (`replaceQueryForBundle` / `OPENCUES_AX_REPLACE_QUERY` → `CueContext.answerReplacesQuery` → fluid-blank's `replaceQuerySpan` whole-buffer WIPE span; `docs/features/answer-replaces-query.md`). Every other app keeps fill-only-the-`_`; the wipe's span comes from the runtime, never the model's `SPAN:` line, and a no-cycling host can't undo it — so don't loosen `replaceQuerySpan`'s single-line + trailing-`_` guards without a replacement recovery path. Aliases: `macos`, `ax`.
...
- `opencues-strategy.md` — internal strategy doc (OpenClaw-model framing, latent-moat self-assessment); public version of the framing will live in a future `GOVERNANCE.md`
...
- **Replace the domain-niche shipped cues (`legal`, `medical`, `financial`) with more general-purpose ones.** Professional-niche domains are poor first-run examples; design a replacement set with broad appeal (candidates: tone/formality, conciseness, inclusive-language) before going public. The removal is a cross-repo sweep, not a folder delete: this repo has ~80 files referencing the trio (defaults/README + the three `defaults/cues/*/CUE.md` folders, `docs/features/*` + `docs/architecture/*` + glossary + configuration + overview, `spec/conformance/` READMEs and any fixtures, the CLI `new` cue template at `packages/opencues-cli/src/templates/new/cue.md`, and `cues-md.test.ts`; regenerate the list with `grep -rl 'legal\|medical\|financial' defaults/ docs/ spec/ packages/ integrations/`), and the **website repo** references them in `faqs.html`, `faq/what-is-a-cue.html`, `md/population/{features,open-standard,changelog}.md`, the inline-cues blog post (html + md), and `md/drafts/blog-posts/*`. Identify and delete/update every instance in both repos in the same pass. Add path-scoped patterns (e.g. `cues/legal`, `name: medical`) to `scripts/lint-legacy-names.sh` `BANNED_PATTERNS` in the removal PR — NOT the bare words, which false-positive on ordinary prose ("legal action" in LICENSE).
...
| `packages/opencues-core/` | `@opencues/core` | 0.32.0 | private |
| `packages/opencues-runtime/` | `@opencues/runtime` | 0.26.0 | private |
| `packages/opencues-cli/` | `opencues` (real CLI) | 0.2.55 | private |
...
| `integrations/mac/` | `@opencues/mac` | 0.4.0 | private |
...
- **Adding a dependency to `@opencues/runtime`'s `dependencies`?** Add it to `RUNTIME_DEPS` in `packages/opencues-cli/src/lib/stage-runtime-deps.cjs`. The self-owned hosts (mac / apple-notes / shell) stage the runtime **dist-only**, so anything not in that list is unresolvable at runtime on those hosts — and the failure surfaces as a warn from whatever lazy-requires it, not as an install error. Mark it `optional: true` only when the consuming code genuinely degrades without it (isolated-vm does; acorn doesn't). Then boot the daemon and grep for `Cannot find module`.
...
| A staged host bundle's TRANSITIVE deps are missing — the feature that needs them degrades to a warn, so the host looks healthy | July 2026 — `gh-issues` (the shipped reference JS blank) failed on mac with `Cannot find module 'acorn'`; built-in and `.sh` blanks kept working so nothing looked wrong. Three stacked causes, each hidden behind the one in front: (a) mac/apple-notes/shell stage `@opencues/{core,runtime}` **dist-only**, so the runtime's own `acorn`/`acorn-walk` were unresolvable (pnpm keeps them in the store, not hoisted to the repo root); (b) the subprocess fallback runs `~/.opencues/vendor/user-blank-runner.cjs` with `NODE_PATH=<runner-dir>/node_modules`, and shell + opencode's `setup.sh` copied isolated-vm from `$OPENCUES_ROOT/node_modules/isolated-vm` — **a path a pnpm workspace never has** — so that dir had been empty since 2026-07-06 and the fallback was dead on EVERY host; (c) copying isolated-vm alone still failed on `node-gyp-build`, which it needs to find its prebuilt binary. | One shared `packages/opencues-cli/src/lib/stage-runtime-deps.cjs` (`stageRuntimeDeps` + `vendorUserBlankRunner`): resolves by REAL resolution rather than an assumed layout, copies **transitively** (a dep's own `dependencies`) with `dereference` so a pruned pnpm store can't break the copy, and is called from mac's `install.cjs` + apple-notes/shell/opencode's `setup.sh`. Required deps (`acorn`, `acorn-walk`) THROW when unresolvable — shipping a bundle whose JS user-blank loader is dead is worse than failing loudly; `isolated-vm` stays best-effort. **When you add a dependency to `@opencues/runtime`, add it to `RUNTIME_DEPS`** or every self-owned host silently loses whatever needs it. | `stage-runtime-deps.test.cjs` (7 hermetic cases, incl. `require()` from a synthesised dist-only bundle and the isolated-vm → node-gyp-build transitive case); boot-smoke the daemon and grep for `Cannot find module` |
