...
- **docs/architecture/session-contradiction.md** ⚠️ Canonical reference for `session-contradiction-mode` — the Claude-Code-only "you're contradicting a decision you made earlier this session" cue. A DIFFERENT engine from `contradiction-cues-mode` (which is deterministic — model routes, runtime computes): this one is a two-stage WATCHLIST matcher. **Stage A (slow producer):** `opencues extract-commitments <transcript_path>` (`packages/opencues-cli/src/commands/extract-commitments.cjs`), kicked fire-and-forget by the CC statusline (`highlight-statusline.sh` `_oc_kick_commitments`, CLI baked in by `setup.sh`), distils the CC session transcript into a terse commitments watchlist → `~/.cues/session-commitments.json` (categories: stack/architecture/constraint/memory/scope/decision). Only user+assistant PROSE reaches the producer (tool I/O + thinking dropped in `extractTranscriptTurns` — the data-minimization boundary). **Stage B (fast matcher):** `SessionContradictionSource` (`packages/opencues-core/src/contradiction/session-contradiction-source.ts`) runs ONE debounced whole-buffer LLM call against the watchlist (in the SYSTEM msg → cerebras prefix-cache), emits a passive `sentence-cue:session-contradiction` result (priority 88). Two grounding invariants: the flagged quote must be an exact buffer substring AND the cited commitmentId must be on the watchlist (else dropped) — the model can't invent a span or cite a phantom commitment. Catalog flows via `CueContext.sessionCommitments` (calendar-context pattern): `buildSessionCommitmentsIngest` (`boot-common.ts`) reads the snapshot on a timer into a live holder, CC boot passes it as `ResolverOptions.sessionCommitments`, resolver forwards it gated by the scalar. Render rail reused (passive, never auto-splices; `Ctrl+Alt+↑` applies the reconciled rewrite; `inline-cues-mode` governs inline vs statusline). OFF by default; runtime-only knob (NOT a spec change). Adapter-band boot edit is OUTSIDE srcHash — must run `setup.sh` to deploy the ingest + statusline bake. User-facing summary: `docs/features/session-contradiction.md`.
