...
- **Edit a TransformBlank prompt (`FUSED_SYSTEM` or any `P1_EXTRACT`/`P1_5_RESOLVE`/`P2_APPLY`/`P2_GENERATIVE`/`P3_VERIFY` constant in `transform-blank-source.ts`)?** The fused and 3-pass prompts are TWO encodings of the SAME behaviour, picked per-provider — fused is the default for most users, 3-pass is groq. A rule added to one MUST be mirrored into the other (or recorded as an intentional asymmetry). Re-run BOTH `prod.ts --mode fused` and `--mode 3-pass`. The bench drives the bare source — catalog-induced bugs (identity/blank-context on) only show in the agentic harness. Full parity table + known gaps + checklist: `docs/architecture/transform-blank.md` § "Two-path parity". The bold bug (PR #195) was a 3-pass-only rule silently missing from fused.
