...
- **docs/architecture/keyword-window.md note** — the keyword window (`packages/opencues-core/src/keyword-window.ts`, `keywordInWindow` / `lineOfWords`) is now ALWAYS LINE-SCOPED: a keyword claims a `_` when it's on the same line. The per-blank `blankProximity` knob and the `blank-intent-mode` gate-gated switch were both retired. It's the single shared predicate routed through by the FIVE claim/cede sites (`BlankFill.matchKeyword` + `BlankSource` claim + `FluidBlank`/`TransformBlank`/`ConfigIntent` cede), so they can't drift. Shaped blanks bypass the window entirely (claimed by their shape).
