...
| `word-cues-mode`, `transform-blank-mode`, `replace-parse-mode`, `sentence-cues-mode`, `fluid-config-mode`, `undo-mode`, `ambient-context-mode`, `max-thinking` | Per-feature enable gates + reasoning-effort budget are reference-runtime knobs — a second runtime could ship any subset always-on. (`replace-parse-mode` gates transform-blank's parallel splice fast-path — pure dispatch strategy, invisible at the file-format level.) |
