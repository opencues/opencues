...
export class AdvisoryState {
  ...
  /** The advisory NEAREST the cursor — the one whose span contains the cursor,
   *  else the one whose span edge is closest (min char distance). Ties break to
   *  the earlier-emitted (higher-priority) one. null when there are none. This
   *  is the single advisory the statusline shows (only surface — no in-text
   *  marker), so "nearest" keeps it relevant as the cursor moves. */
  nearestTo(charOffset: number): Advisory | null {
    let best: Advisory | null = null;
    let bestDist = Infinity;
    for (const a of this._advisories) {
      const dist = charOffset < a.spanStart ? a.spanStart - charOffset
        : charOffset >= a.spanEnd ? charOffset - a.spanEnd + 1
        : 0;
