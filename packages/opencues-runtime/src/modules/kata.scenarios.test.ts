// Scenario tests for KataCoach — multi-step user journeys against a
...
function makeHarness() {
  ...
  const adapter = {
    ...
    readDir: async (path: string) => path.endsWith('/katas')
      ? [{ name: 'demo', isDirectory: true }]
      : null,
    readFile: async (path: string) => path.endsWith('demo/KATA.md') ? KATA_MD : (files.get(path) ?? null),
  ...
  const coach = new KataCoach(
    adapter as never,
    configLoader as never,
    {
      katasDirs: ['/cues/katas'], resolveLLM, log: () => { /* noop */ },
      progressFile: '/cues/kata-progress.json',
...
describe('KataCoach journeys (deterministic contracts)', () => {
  it('start → done → done → done completes; control spans consumed', async () => {
    const h = makeHarness();
    await h.type('start kata 1 _');
    expect(h.events.map(e => e.type)).toContain('kata.started');
    ...
    expect(h.events.map(e => e.type)).toContain('kata.completed');
    const advances = h.events.filter(e => e.type === 'kata.step-advanced');
  ...
  it('Esc ×3 exits deterministically with countdown — zero LLM involvement', async () => {
    ...
    h.key('escape');
    expect(h.coach.active).toBe(true);
    expect(h.coach.status()?.coach).toContain('Esc ×2 more to exit the kata');
    h.key('escape');
    expect(h.coach.active).toBe(true);
    expect(h.coach.status()?.coach).toContain('Esc ×1 more to exit the kata');
    h.key('escape');
    expect(h.coach.active).toBe(false);
    const stopped = h.events.find(e => e.type === 'kata.stopped');
    expect(stopped?.body?.reason).toBe('escape-key');
    // Transient notice carries the resume phrase.
    expect(h.coach.status()?.coach).toContain('start kata 1 _');
  ...
  it('meta-flavoured Escape counts toward the hatch (terminal ESC-prefix encoding)', async () => {
    ...
    expect(h.events.find(e => e.type === 'kata.stopped')?.body?.reason).toBe('escape-key');
  });
  ...
  it('Esc counter resets after the 2.5s window and on other keys', async () => {
    ...
    h.key('escape');
    expect(h.coach.status()?.coach).toContain('Esc ×2 more to exit the kata'); // reset, not ×1
  ...
  it('escape presses land in the trace (katas can teach Esc)', async () => {
    const h = makeHarness();
    await h.type('start kata 1 _');
    h.key('escape');
    h.key('escape');
  ...
  it('stop kata _ deactivates; katas-mode: off gates everything', async () => {
    ...
    expect(h.coach.active).toBe(false);
    expect(h.events.filter(e => e.type === 'kata.stopped').at(-1)?.body?.reason).toBe('user');
  });
  ...
  it('resolver suppression covers active mode AND control-phrase typing', async () => {
    ...
    expect(h.coach.shouldSuppressResolve('start kata 1 _')).toBe(true); // mid-phrase, inactive
    await h.type('start kata 1 _');
  ...
  it('coach unreachable (2 consecutive network failures) → offline hint; recovery clears it', async () => {
    ...
    expect(h.events.filter(e => e.type === 'kata.tick' && e.body?.error).length).toBe(2);
    void failing;
  });
  ...
  it('idle nudge (no LLM): fires at the window, escalates once with skip _, then stays quiet; activity resets', async () => {
    ...
    expect(h.events.filter(e => e.type === 'kata.nudge')).toHaveLength(0);
    // Full idle window from the last activity → nudge 1.
    await vi.advanceTimersByTimeAsync(2_000);
    let nudges = h.events.filter(e => e.type === 'kata.nudge');
  ...
  it('progress persists: stop mid-way → resume with journal; completion recap carries the next link', async () => {
    ...
    const saved = JSON.parse(h.files.get('/cues/kata-progress.json')!);
    ...
    expect(notice?.coach).toContain('start kata demo-two _');
    expect(notice?.coachSegments?.some(s => s.command && s.text === 'start kata demo-two _')).toBe(true);
    const done = JSON.parse(h.files.get('/cues/kata-progress.json')!);
  ...
  it('voice: speaks step advances + completion only when kata-voice is on', async () => {
    const h = makeHarness();
    await h.type('start kata 1 _');
    await h.type('done _');
    expect(h.spoken).toHaveLength(0);          // default off
    h.settings.set('kata-voice', 'on');
    await h.type('done _');
    expect(h.spoken.at(-1)).toContain('Step 3 of 3');
    await h.type('done _');                     // completes
    expect(h.spoken.at(-1)).toContain('Kata complete');
    expect(h.spoken.at(-1)).toContain('demo-two');
  });

  it('unknown kata → not-found event + transient catalogue notice', async () => {
    const h = makeHarness();
    await h.type('start kata 99 _');
    expect(h.coach.active).toBe(false);
    const nf = h.events.find(e => e.type === 'kata.not-found');
