...
export class Resolver {
  ...
  constructor(
    ...
    /** The fluid ADVISORY channel. Validator-class results (contradiction)
     *  are routed here as passive statusline annotations instead of being
     *  registered as cycle-cue DynDefs, so they coexist with the cue that
     *  owns the span rather than evicting it. Rebuilt every resolve pass
     *  (self-clearing). Omit to disable advisory surfacing. */
  ...
  async resolveAndApply(text: string, opts: { allowBlanks?: boolean } = {}): Promise<void> {
    ...
    // Fluid advisory channel — validator-class results (contradiction)
    // accumulate here and land on AdvisoryState after the pass, instead of
    // registering DynDefs. Rebuilt every pass so advisories self-clear.
    ...
    for (const r of applicableResults) {
      ...
      if (isSentenceCue && r.alternatives.length >= 2 && isMultiWordSpan) {
        ...
        // above already re-grounded the span against the live buffer, so the
        // advisory points at real text. `alternatives = [flaggedQuote,
        // correction]`; a correction distinct from the quote is offered.
        if (r.validator) {
        ...
        for (const [key, def] of this.dynDefs.entries()) {
          ...
          if (start < def.spanEnd && def.spanStart < end) {
            ...
            // (Validator-class results never reach here — they're routed to the
            // advisory channel above and don't register a DynDef at all.)
    ...
    // Publish the fluid advisory set for this pass. An empty array clears
    // stale advisories (the self-clear). Force a paint when the set is or was
    // non-empty so the statusline updates even if no DynDef was written.
