...
import { DictionaryControl } from './dictionary';
...
describe('DictionaryControl', () => {
  it('returns empty string when no word can be picked from keyword/context', async () => {
    const ctl = new DictionaryControl({ fetchFn: fetchOk(STUB_DEF('x')) });
  ...
  it('extracts the longest content word from keyword + context', async () => {
    ...
    const ctl = new DictionaryControl({ fetchFn: fetchFn as unknown as typeof fetch });
  ...
  it('returns the first definition truncated to 100 chars', async () => {
    ...
    const ctl = new DictionaryControl({ fetchFn: fetchOk(STUB_DEF(long)) });
  ...
  it('returns "<word>: not found" for HTTP 404', async () => {
    const ctl = new DictionaryControl({ fetchFn: fetchOk({}, 404) });
  ...
  it('caches within TTL — second call same word does not re-fetch', async () => {
    ...
    const ctl = new DictionaryControl({
  ...
  it('returns "<word>: error" on fetch throw', async () => {
    ...
    const ctl = new DictionaryControl({ fetchFn });
