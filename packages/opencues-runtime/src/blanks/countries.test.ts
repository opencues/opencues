...
import { CountriesControl } from './countries';
...
describe('CountriesControl', () => {
  it('returns "<fact>: no country" when keyword has no country context', async () => {
    const ctl = new CountriesControl({ fetchFn: fetchOk(FRANCE) });
  ...
  it('returns first 3 languages joined', async () => {
    ...
    const ctl = new CountriesControl({ fetchFn: fetchOk(india) });
  ...
  it('caches on country (multiple facts → single fetch)', async () => {
    ...
    const ctl = new CountriesControl({ fetchFn: fetchFn as unknown as typeof fetch, cacheTtlMs: 60_000 });
  ...
  it('returns "<country>: not found" for HTTP 404', async () => {
    const ctl = new CountriesControl({ fetchFn: fetchOk({}, 404) });
