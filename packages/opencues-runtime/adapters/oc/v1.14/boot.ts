...
export function boot(host: HostInfo): BootResult {
  ...
  if (process.env.OPENCUES_BRIDGE === '1') {
    startEventBridge({
      ...
      // (src/bridge-render-bindings.ts), so the bands cloned from this one
