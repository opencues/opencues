...
export function boot(host: HostInfo): BootResult {
  ...
  const resolverOpts: {
    ...
    formatLLMErrorAsSubstitute?: (reason: 'invalid-api-key' | 'network' | 'rate-limit' | 'endpoint-not-found' | 'model-not-found' | 'bad-request', err?: Error) => string;
  } = {
  ...
  if (true) {
    ...
    const resolver = new Resolver(adapter, hlState, dynDefs, configLoader, Object.assign(resolverOpts, {
      ...
      formatLLMErrorAsSubstitute: (reason: 'invalid-api-key' | 'network' | 'rate-limit' | 'endpoint-not-found' | 'model-not-found' | 'bad-request'): string => {
