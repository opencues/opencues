...
import { deriveReplaceValue } from './replace-value';
...
export interface TransformBlankSourceConfig {
   ...
   *  request instead of the chat call (decision layer): the `replace` leg
   *  names the target and the command, and the value comes from the fused
   *  rewrite's diff. Absent → the chat detector as before. */
...
export class TransformBlankSource implements CueSource {
  ...
  private async runFusedAndBuild(
    ...
    const replaceDecisionPromise: Promise<ReplaceVerdict | null> | null = this.replaceParse && this.decisions
      ? (async () => {
          const d = await this.decisions!.replace(inputForLLM, { signal: context.signal, log: (m) => this.log(`TransformBlank ${m}`) });
          this.log(`TransformBlank replace-decide: ${d ? `${d.summary} → "${preview(d.target)}" / "${d.command}"` : 'fused merge'}`);
          return d;
        })().catch((e: unknown) => {
