...
module.exports = function version(argv, ctx) {
  ...
  console.log(`opencues v${ctx.pkg.version}`);
  console.log('');
  ...
  for (const h of HOSTS) {
    ...
    const compat = p.compatibility
      ...
      : '(unspecified)';
    rows.push([p.name || `@opencues/${h}`, `v${p.version || '?'}`, compat]);
  }
  for (const [name, ver, compat] of rows) {
    console.log(`  ${name.padEnd(20)} ${ver.padEnd(10)} ${compat}`);
  }
  console.log('');
  console.log('Internal libraries:');
  for (const lib of ['opencues-core', 'opencues-runtime']) {
    const pkgPath = path.join(ctx.REPO_ROOT, 'packages', lib, 'package.json');
    if (!fs.existsSync(pkgPath)) continue;
    const p = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
    console.log(`  ${(p.name || lib).padEnd(20)} v${p.version || '?'}`);
  }
};
