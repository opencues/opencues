...
### Fixed — macOS Ghostty / iTerm2: Ctrl+Option+arrow now works in OC + shell

A tester reported `Ctrl+Alt+arrow` doing nothing on macOS. `cat -v` testing produced a more nuanced result than the original bug report suggested: **Mac Terminal.app cannot transmit the Ctrl bit for Ctrl+Option+arrow at all** — `\x1b\x1b[A` is identical to plain Option+arrow with Option-as-Meta on, no software fix downstream of the byte stream can recover it. Install path is now blunt about this (use Ghostty or iTerm2). For users already on Ghostty / iTerm2 (which transmit modifier-encoded CSI like `\x1b[1;7A`), this PR fixes a real bug in OC + shell:

- **`integrations/shell/src/bootstrap.ts`** AND **`integrations/opencode/patches/opencuesBootstrap.ts`** modifier coalesce was `alt: !!evt.option || !!evt.alt`. OpenTUI (which both hosts use) parses Ghostty/iTerm2's modifier-encoded `\x1b[1;7A` as `{ctrl: true, option: true, meta: true}` — but without `meta` (and on some paths `option`) in the coalesce, the runtime saw `alt: false` and the `ctrl-alt` keymap matcher never fired. CC adapter at `packages/opencues-runtime/adapters/cc/v2.1/adapter.ts:339` already had the right coalesce (`raw.alt || raw.option || raw.meta`) and worked correctly. Shell and OC are now aligned. ([@opencues/shell](integrations/shell/) 0.1.2 → 0.1.3, [@opencues/opencode](integrations/opencode/) 0.1.1 → 0.1.2)
- **Mac Terminal.app: NOT fixable from the integration side.** Terminal.app's `\x1b\x1b[A` for Ctrl+Option+arrow carries no Ctrl byte at all (Mission Control eats `\x1b[1;5A`-style sequences before they leave the OS). The runtime sees `ctrl: false`; the `ctrl-alt` matcher (correctly) does not fire. `docs/install.md` is now blunt about this — Terminal.app users must install Ghostty or iTerm2.
- **gemini-cli is independently broken on macOS even with Ghostty/iTerm2**: Gemini's own `KeypressContext.tsx` parser at line 585 reads `alt` from the CSI modifier byte (`!!(modifier & 2)`); it works for modifier-encoded CSI like `\x1b[1;7A` (alt=true, ctrl=true), so Ghostty / iTerm2 ARE fine. The double-ESC drop only affects Mac Terminal.app users.
- **chrome unaffected**: DOM KeyboardEvent exposes `altKey` directly. Works on any Mac browser.
- **Banner now shows "Ctrl+Option" on macOS** (`packages/opencues-cli/src/commands/run.cjs`) — the chord label that matches the user's physical keyboard.
