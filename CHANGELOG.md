...
- **`opencues install` ends with a key-detection report.** Detected keys print with their source plus the effective default provider (explicit `llm-provider:` scalar, else the auto-route pick). With zero keys it points at `opencues set-key` and — when the `claude` / `codex` binary is on PATH — names the zero-key subscription option (`llm-provider: claude-code-cli` / `openai-subscription`). Suggested, never auto-applied: the CLI-daemon transport is markedly slower than the API providers, so switching silently would regress latency-sensitive word-cues. Previously the install finished saying nothing about keys and the "every LLM feature is inert" warning only surfaced if the user happened to run `doctor`.
