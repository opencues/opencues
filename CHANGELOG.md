...
- `isGroundedRewrite` accepted `i want to work on the hotfix … --worktree` and `Press Ctrl+R to reverse-search for <draft>` because they keep every word of the draft. Both are advice to the person, not a prompt: a rewrite that adds a launch flag (`-p`, `--sandbox`) or a keybind (`Ctrl+R`, `Shift+Tab`, `Esc`, `F12`) the draft did not have now collapses to an advisory. `@path` and `!cmd` stay, since a prompt can carry them. Seen live on gpt-oss-120b for `--sandbox`, `--worktree`, `Ctrl+R` and `Ctrl+T`.
