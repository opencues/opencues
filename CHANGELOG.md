...
### Changed — Resolver cancels in-flight LLM calls when superseded (saves 30-50% provider $$$ during noisy typing)

Pre-change: every `resolveAndApply` increments an internal `_generation` counter. If a newer call fires while a prior one is in flight, the prior LLM call runs to completion and its result is dropped on generation mismatch at apply time. The result is correct but the provider round-trip is wasted — token cost burned, rate-limit pressure raised, no user-visible value. During fast typing in a `_`-bearing buffer (`make this more formal _ hello there`), every keystroke past the first kicked off transform-blank EXTRACT + fluid-blank FUSED in parallel; the first few rounds all returned and were silently discarded.

Post-change: each `resolveAndApply` owns a fresh `AbortController`. Bumping the generation on a new call aborts the prior controller's signal; every source that issued an LLM call via `dispatchChat` was passed `context.signal` and forwards it to `httpAdapter.post`. The Node HTTP adapter destroys the request mid-flight; the chrome FetchHttpAdapter rejects the SW-relay promise locally (full SW plumbing is a follow-up). `AbortError` on supersede is recognised and silenced at every layer — `withFallback` no longer retries on abort, the resolver's outer catch suppresses the log line, the SW path stays clean.

- **`@opencues/core` (0.3.0 → 0.3.1)** — `HttpAdapter.post` gains an optional 4th `options?: { signal?: AbortSignal }` arg (backwards-compatible). `HttpAdapterShape` mirrors. `CueContext` gains `signal?: AbortSignal`. `dispatchChat` accepts `ctx.signal` and forwards to `httpAdapter.post`. Every shipped source (SentenceCue, FluidBlank, TransformBlank, ConfigIntent) threads `context.signal` into its private `callLLM`. All 6 `callLLM` sites inside TransformBlank (P1 EXTRACT, P1.5 RESOLVE, P2 GENERATIVE, P2 APPLY, P3 VERIFY, FUSED) pass it through. `withFallback` skips the failover retry on abort — chaining to the fallback after the user moved on would just burn the fallback's quota too.
- **`@opencues/runtime` (0.2.3 → 0.2.4)** — `Resolver` owns a `_inFlightController: AbortController | null` field, replaced on each `resolveAndApply`. Aborts the prior on entry. Forwards `controller.signal` in the `_resolver.resolve(...)` context. Module-level `isAbortError` helper recognises the AbortError shape so the outer catch doesn't log it as a fault.
- **`@opencues/chrome` (0.2.1 → 0.2.2)** — `FetchHttpAdapter.post` accepts options, race-rejects the SW-relay promise on abort. Full provider-side cancellation (SW telling the host to drop the request) is a follow-up.
- **Node HTTP adapter** (`packages/opencues-core/node-http-adapter.js`) — `post()` accepts options, wires the signal to `req.destroy(err)` on abort, detaches the listener on `close` so we don't leak handlers across requests.
- **3 new regression tests** in `packages/opencues-runtime/src/modules/resolver.test.ts`: (1) a fresh AbortSignal lands in every resolve's ctx (one per call, not shared); (2) a newer `resolveAndApply` aborts the prior signal mid-flight; (3) an AbortError on supersede does NOT log as `Resolver.resolve threw` — abort is an expected control-flow event, not a fault.
- **Limit (v1)**: CLI-transport providers (`claude-code-cli`, `openai-subscription`) ignore the signal — `dispatchChat` short-circuits to `invokeCli` before reaching the HTTP path. Subprocess cancellation needs `child_process.kill` plumbing through `invokeCli`'s contract. Acceptable for v1 because the wasted-token cost dominates at HTTP providers (Groq, Cerebras, OpenRouter), not at subscription transports.
