...
### Fixed — pin tweakcc to `v4.3.0`; tweakcc 4.3.1 corrupted the CC native binary (`@opencues/claude-code` 0.2.7 → 0.2.8)

`opencues install claude-code`'s `setup.sh` cloned `github.com/Piebald-AI/tweakcc` at **HEAD**, which pulled the day-old **4.3.1**. 4.3.1 regressed the native-binary `.bun`-section repack: the repacked `claude.exe` bloats 247MB → 411MB and crashes at startup with `TypeError: Expected CommonJS module to have a function wrapper` — Bun can't load the corrupted module graph, so OpenCues never boots, no `[cc]` log lines appear, and the agentic harness bridge never arms. The extract + JS patch were fine (+40KB bootstrap); only tweakcc's repack broke. **Fix:** `setup.sh` now clones then `git checkout`s an exact tag read from `compat.json:tweakcc-pin` (`v4.3.0`, the newest tag that repacks correctly — the regression is in the 14 commits after it) instead of HEAD; `TWEAKCC_REF=<tag>` overrides for bisecting a future release. **Guard:** `install.cjs:validateFork` gained a native-binary startup smoke — after patching, `spawnSync(claude.exe, ['--version'])` must exit 0 and print `Claude Code`. Previously `validateFork` only `require()`d the dist modules under Node and never ran the actual Bun binary, so it reported the dead fork "healthy". Now a corrupt repack can never ship. Root CLAUDE.md § Common drift-bug patterns documents the class.
