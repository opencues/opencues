...
### Added — chrome inline-note push-down on managed editors (claude.ai/ProseMirror etc.) via a safe margin nudge (`@opencues/chrome` 0.2.110 → 0.2.113)

The push-down that stops the note occluding the line below now works on managed editors too, not just plain contenteditables. A node spacer can't be used there: managed editors (ProseMirror/Lexical/Quill) revert externally-inserted nodes, AND OpenCues doesn't own their send button — a real inserted line would ship in the user's message. Instead, on managed editors the note nudges the span's containing block's `margin-bottom` via inline **style**: layout only, so it can never reach the submitted text (`walkPlainText` reads text, not styles) and creates no editor transaction (no undo-stack entry). If the editor reverts the style on its next redraw the push-down simply doesn't hold and the note floats — no worse than before, never unsafe. The push mode is now a three-way `'node' | 'margin' | 'none'` (`applyDirectives`'s third arg): plain CE → `node`, managed → `margin`, normal inputs → render skipped. The margin path has two branches: when the caret's line has its own block ancestor inside the editor **AND a following sibling to push**, nudge THAT block's `margin-bottom` (correct mid-buffer); otherwise — the last/only line (the usual compose-box case), or no sub-block — grow the EDITOR itself via `padding-bottom`. The sibling gate matters: a bottom margin on the *last* child is swallowed (doesn't grow the parent's box), so on a single-paragraph ProseMirror (claude.ai one-liner) it opened no visible row until this branch took over. Prior inline value is restored verbatim on clear. A debug-mode diagnostic (`[chrome] marginPush …`) reports which path ran + the target tag, so a managed editor that reverts the nudge is visible in `/tmp/opencues.log`. Pinned by 3 new jsdom tests (block-margin nudge, root-padding fallback, exact restore) in `runtime-renderer.test.ts`.
