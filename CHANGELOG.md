...
- **`integrations/shell/src/bootstrap.ts`** AND **`integrations/opencode/patches/opencuesBootstrap.ts`** modifier coalesce was `alt: !!evt.option || !!evt.alt`. OpenTUI (which both hosts use) surfaces Mac's Option key as `meta: true` when the byte stream is ESC-prefixed; without `meta` in the coalesce, the runtime saw `alt: false` and the `ctrl-alt` keymap matcher never fired. The CC adapter at `packages/opencues-runtime/adapters/cc/v2.1/adapter.ts:339` already had the right coalesce (`raw.alt || raw.option || raw.meta`) — shell and OC are now aligned. ([@opencues/shell](integrations/shell/) 0.1.2 → 0.1.3, [@opencues/opencode](integrations/opencode/) 0.1.1 → 0.1.2)
- **gemini-cli unaffected by this fix but has a separate underlying limitation**: Gemini's own `KeypressContext.tsx` parser at line 585 reads `alt` from the CSI modifier byte (`!!(modifier & 2)`) and discards the outer ESC-prefix from a double-ESC sequence. So when Mac Terminal sends `\x1b\x1b[A` for Ctrl+Option+arrow, Gemini's parser produces `{alt: false, ctrl: false, name: 'up'}` — the modifier info is lost upstream of our bootstrap. Not something we can fix from the integration side. Gemini-cli users on Mac Terminal should switch to Ghostty / iTerm2 (which emit modifier-encoded CSI like `\x1b[1;7A` directly), or use `nav-keymap: ctrl-shift` if their terminal forwards that.
- **chrome unaffected**: DOM KeyboardEvent exposes `altKey` directly with no shell-escape parsing in the path.
