...
### Added — tutorial mode prototype: modal guided scenarios with a live LLM coach (`@opencues/runtime` 0.10.0 → 0.11.0)

**Experimental.** New `TutorialCoach` module (`packages/opencues-runtime/src/modules/tutorial.ts`): tutorials are authored as `~/.cues/tutorials/<name>/TUTORIAL.md` (frontmatter + `## Step` sections; per-step `coach:` prose rides into the system prompt verbatim, so fidelity lives in the file, not a schema). `start tutorial <id|name> _` activates a MODAL state that suppresses the entire Resolver (new `ResolverOptions.externallySuppressed` seam — word-cues, fluid/transform-blank, config-intent, sentence-cues all dark until `stop tutorial _`; nothing is written to OPENCUES.md so stop is trivially clean). A debounced (~300ms) coach call on the auditors bucket receives the full script (stable → provider prefix-cache) plus a bounded trace of typed activity — including submit detection via non-empty→empty buffer transitions — and returns `STEP/STATUS/COACH`; the runtime trusts the model with hardcoded floors only (bounds-clamped, never backward, ≤1 step per tick). Detection is seamless — the trace also carries salient KEY PRESSES (passive `onKey(null, …)` observation: tab/shift+tab, escape, arrows, enter-on-empty-buffer, modifier combos), so steps that happen outside the input box (mode toggles, pickers) complete without the user announcing anything; `done`/`next`/`skip _` remain as deterministic escape hatches but tutorials never require them. Coach output is DISPLAY-ONLY (statusline `tutorial` block + step counter; never the buffer, never a side-effect layer). `done`/`next`/`skip _` advance deterministically. Every transition emits structured events (`tutorial.started/tick/step-advanced/completed/stopped`) for the agentic harness. Idle nudges (default 30s, `tutorial-nudge-ms`): proactive, context-aware check-ins, escalating once with `skip _` then going quiet (2-per-stall cap, reset on activity), advisory-only (never advance, never stop); deterministic "Still there?" fallback with no LLM. Lesson journal: one line per completed step (with closing evidence) rides into every coach call as cross-step memory. Shipped example: `defaults/tutorials/claude-code-basics/` (not yet seeded by `seed-configs` — copy manually). OC band wired; CC/gemini/shell/chrome + feature-registry `tutorials-mode` scalar + persistence deferred. Measured on the OC agentic harness: coach ticks avg 307ms (247–410) on cerebras `gpt-oss-120b`; 15/15 scenario steps green; 0 resolver dispatches while modal.
