...
- **`opencues calendar sync`** — the PRODUCER. Fetches every feed → writes `~/.cues/life-context.json` (the one snapshot all hosts read). A total fetch failure never overwrites the last-good file. `add`/`refresh` can trigger it. This replaces per-host pollers with one central refresh.
- **Chrome consumes, never fetches.** `life-context.json` is registry-pushed (`prereqFile` + `pushedBy: ['chrome-host']`), so `opencues sync chrome` + the native-messaging host carry it into the bundle. The bootstrap reads it → `buildLifeContextSnapshot` → a mutable holder passed to the resolver, re-loaded on every `opencues_bundle` storage change. Chrome band gains a `lifeContext` HostInfo field + resolver option (mirrors windows). The calendar `CUE.md` already syncs via the folder-cues walk.
...
- Verified: core/runtime/chrome all typecheck + esbuild clean; `loadLifeContext` present in the bundle; feature-registry alignment green.
...
> **To test (chrome):** `opencues calendar sync` → `opencues sync chrome --wsl` (or run the chrome-host) → copy `dist/`+`manifest.json` to the Windows extension dir → reload the extension + hard-refresh → set `life-context-mode: on` and `sentence-cues-mode: on` → in a contenteditable field type "I'm free on the 23rd" and cycle Ctrl+Alt+Up to see the conflict heads-up. Run `npm run test:e2e:chrome` after building.

### Added — life-context poll cadence + on-demand refresh (`opencues` 0.2.47 → 0.2.48, `@opencues/windows` 0.1.5 → 0.1.6)
...
- **Configurable cadence** — `life-context-poll-minutes` in OPENCUES.md (default **30**, clamped 5–1440). Read live from the file (no rebuild).
- **`opencues calendar refresh`** — forces a fresh, **cache-busting** poll now (appends a rotating `?v=` so Google/etc. skip their server cache). Decoupled + host-agnostic: the CLI writes `~/.cues/.life-context-refresh`; the daemon's cheap 20-s stat-check picks it up and re-polls. Verified live end-to-end.
- **No restart for add/remove** — the poller re-reads `life-context-feeds.txt` every poll, so `opencues calendar add/remove` takes effect on the next cadence (or immediately via `refresh`). (First-ever feed while a host is already running still needs one restart to start the poller.)
- The forced pull only cache-busts on `refresh`; the normal cadence uses plain URLs (respects caching, avoids throttling).

### Added — `opencues calendar` CLI: manage life-context feeds (`opencues` 0.2.46 → 0.2.47)

A first-class CLI for the calendar feeds instead of hand-editing `~/.cues/life-context-feeds.txt`:

- **`opencues calendar add <url>`** — validates BEFORE writing: fetches the URL and confirms it's a real iCalendar feed (rejects HTTP errors AND 200s that aren't iCal — no `VCALENDAR` block), then reports "N events, M in the next 60 days, next: …". Rejects non-URLs, guards duplicates, accepts `webcal://`. `--no-verify` skips the network check. Warns if `life-context-mode` is off.
...
### Added — life-context `.ics`/webcal producer: real calendars via feed URL (Phase 1b) (`@opencues/core` 0.26.0 → 0.27.0)

The first REAL life-context producer — point it at an iCalendar feed URL and your upcoming events flow into both the `_` blank (`am i free…`) and the calendar-conflict cue, self-refreshing. One `.ics` parser covers **Luma, Google, Outlook/M365, Apple iCloud, Fastmail, Meetup, and any feed** — the near-universal calendar export — so almost any calendar is a drop-in, no per-provider OAuth for public/secret-URL feeds.

- **New `@opencues/core/ics.ts` — `parseIcs(text, {window})`** (pure, no network). Handles VEVENT SUMMARY/DTSTART/DTEND/DURATION/LOCATION/RRULE, line unfolding + TEXT unescaping, times in UTC (`…Z`, what Luma emits) / named zone (`TZID`, via `Intl`) / floating / all-day (`VALUE=DATE`, exclusive DTEND collapsed), and DAILY/WEEKLY recurrence expansion (INTERVAL/COUNT/UNTIL/BYDAY) bounded to the window (other FREQs → master instance). Normalises everything to local wall-clock `YYYY-MM-DDTHH:MM` — the shape `buildLifeContextSnapshot` + the minute-of-day math already use. 12 unit tests incl. a Luma-shaped feed; live-verified against Google's public UK-holidays ICS (95 KB / 244 VEVENTs parsed).
- **Windows daemon poller** — reads feed URLs from `<CUES_HOME>/life-context-feeds.txt` (one per line, `#` comments, `webcal://` auto-converted), fetches every 15 min + on boot (15 s timeout per feed), parses, dedupes across feeds, and refills the snapshot in place. A total fetch failure **never wipes** the last-good calendar; the blank + cue pick up refreshes live (no restart). Precedence: **live feeds > manual `life-context.json` > Phase-1a fixture**. Live-verified end-to-end in the daemon.
- **The seam held** — zero consumer changes. The producer just writes the same snapshot shape; fluid-blank + the conflict cue + dehydration all worked unchanged. Template: `defaults/life-context-feeds.txt` (with Luma / Google / Outlook how-to-subscribe notes).
...
- **Self-inert** when `life-context-mode` is off (no catalog forwarded → the cue cedes, no wasted LLM calls). New `renderLifeContextForCue` (data + now-anchor, no availability-answer rules — the CUE.md prompt owns the task). Bench `tests/benchmarks/fluid-blank-ambient/life-context-cue-test.ts`: **24/24** on groq gpt-oss-120b (4 conflict → flagged with the right event, 4 no-conflict → ceded).
- **Known limitation:** currently requires BOTH `sentence-cues-mode: on` (the gate that builds any sentence-cue) AND `life-context-mode: on`. Decoupling the calendar cue from `sentence-cues-mode` is a follow-up.

### Added — `life-context`: ingested calendar as reasoning context for fluid-blank (Phase 1a) (`@opencues/core` 0.24.0 → 0.25.0, `@opencues/runtime` 0.20.0 → 0.21.0, `@opencues/windows` 0.1.4 → 0.1.5)

The first slice of life-context (see `docs/architecture/life-context.md`): the system holds a **bounded, periodically-ingested** snapshot of the user's calendar and lets fluid-blank **reason** over it — `am i free thursday _` → an answer computed from the event times. Ingest-on-a-timer, never invoke-per-keystroke; the worst case is a bad suggestion, never an action. Phase 1a proves the `ingest → catalog → reason → hydrate` path with a **fixture** calendar (zero network); Phase 1b swaps in a real calendar-feed (`.ics`) fetch. (No MCP — the ingest is plain iCalendar feeds.)

- **New `@opencues/core/life-context.ts`** — the FOURTH context catalog, but a different SHAPE from the first three. identity/blank/system-context are *substitution* catalogs (the LLM emits a token, the runtime swaps a value it never reasons over). Life-context is a *reasoning* catalog: event **times reach the LLM in the clear** (a busy interval is not PII and is the reasoning substrate), and only event **titles are dehydrated** to `[EVENT N]` tokens hydrated locally via the shared `postProcessContext` path. `buildLifeContextSnapshot` + `renderLifeContextCatalog`.
