...
### Fixed — chrome: `_` silently ignored on non-cycling `<input>`/`<textarea>` (first `_` in a new field, or a stacked `_` after a fill) (`@opencues/runtime` 0.28.2 → 0.28.4, `@opencues/chrome` 0.2.90 → 0.2.93)

On a plain (non-cycling) chrome field — Luma's form fields, any normal `<input>`/`<textarea>` — a `_` was intermittently dropped with no log line: the first `_` in a freshly-focused field would do nothing, and deleting + retyping made it work; a second `_` in the same field after a fill likewise no-op'd. Root cause: the chrome adapter's `lastSeenText` — the `previousText` baseline the resolver's explicit-`_` gate compares each new `_` against — went stale. Normal-input writes go through `writeNormalInputValue`, which dispatches a synthetic `input` event with `isTrusted=false`; content.ts drops those for security, so the write never round-trips to the runtime. And `runtimeRender()` early-returns for normal inputs (no CSS Custom Highlights), so the render path that also refreshes the baseline never fired. With both refresh paths skipped, `lastSeenText` froze at the just-typed `_` and even carried across a focus change into the next field. The gate then read `blankJustTyped = endsWith('_') && !prevEndsWith('_')` → false AND the count-diff fallback `newU > prevU` → false, discarding the trigger *before* the gate's own BLOCKED log — hence completely silent. Deleting + retyping cleared the stale baseline, which is why a retry worked; a stray `selectionchange` refreshing the baseline is why it "sometimes worked."

This is the "stacked-blank after substitute silently no-ops" class (`resolver.ts` § explicit-`_` gate): the `e.previousText` gate is correct, but relies on the adapter keeping `lastSeenText` current, which the normal-input path didn't. Two-part fix in the chrome adapter band, both pure state writes (no event emit, no re-entrancy):

1. **Wrap every runtime write** (`setText` AND `pushText`) in `adapters/chrome/v1/boot.ts` to update `lastSeenText`/`lastSeenCursor`. `pushText` is the ACTUAL fill path (resolver + BlankFill both prefer it when the adapter provides it — chrome does), so an earlier wrap of `setText` alone missed every real fill. Managed-editor writes already round-tripped a trusted `input` event, so only the broken normal-input case changes.
2. **Sync the baseline to the focused field on every focus change** — new `BootResult.noteBufferText(text, cursor)` seam, called from `publishTarget` alongside `resetBufferState`, set to the newly-focused field's actual content (empty → `''`, existing text → that text). This is the robust invariant: even a *failed* fill (which leaves a stale `_` with nothing to overwrite it) can no longer bleed into the next field's first `_`.

Verified live on Luma across fresh fields and repeated `_`.
