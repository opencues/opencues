...
### Added — **Undo / Redo**: language-invariant `undo _` / `redo _` reverts anything OpenCues did (`@opencues/core` 0.19.2 → 0.20.2, `@opencues/runtime` 0.17.0 → 0.18.2)
...
**Undoing a fill no longer re-arms the trigger `_`** (`@opencues/runtime` 0.18.1 → 0.18.2). Reverting a `_`-triggered fill (fluid-blank lookup, keyword blank) used to restore the buffer verbatim — including the trigger `_`, a live trigger that re-fires the fill on the next keystroke (the re-fire loop). A new `fillSplice` records the undo direction WITHOUT the trigger: `capital of france _` → `Paris`, then undo → `capital of france` (no `_`, no dangling separator). The relocation anchor is the preceding WORD only (not its trailing whitespace — the command-span wipe eats that, so a trailing-space anchor wouldn't be found at redo time), so redo still re-applies the value. Self-guards to plain `diffSplice` for any change that isn't a lone-`_` fill (word-cue swaps, whole-buffer rewrites unaffected).
