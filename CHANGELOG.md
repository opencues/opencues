...
### Added — input-box context → auto-populated `_` on Windows (`@opencues/core` 0.21.0 → 0.22.0, `@opencues/runtime` 0.18.6 → 0.19.0, `@opencues/windows` 0.1.1 → 0.1.2)

Extends the existing `ambient-context-mode` feature to the Windows host and adds **app-aware output steering** so a `_` in a native input box is reshaped to that app's expected input. The Windows shim already read the focused field's UIA metadata (control `Name`, `HelpText`, foreground window title, process name) for its attachability + surfaces catalog; it now forwards them on the `focus` event, the daemon maps them to `AmbientContext` (`buildAmbientFromFocus`), and the `windows` adapter's new `getAmbientContext()` hands them to `FluidBlankSource` — chrome parity on Windows, for free.

New optional `AmbientContext.app` field (the native equivalent of chrome's page/site — the focused process name, e.g. `explorer`). When present, `renderAmbientBlock` appends a short **app-steer sentence to the trusted framing of the USER message** (never the shared `FUSED_SYSTEM_PROMPT`): a file-manager / file-explorer search box → a bare file-search token (`my tax pdfs _` → `*.pdf`, `the downloads folder _` → `Downloads`), with a hard anti-empty floor (echo the user's own words rather than blank the answer).

**Field-only, by construction:** only the focused field's own metadata + the window title are read — never a sibling control's value (Explorer's address-bar path is deliberately NOT read; the window-title folder-leaf is the safe substitute). The steer lives in the user message emitted only when `app` is present, so every app-less prompt (all of chrome + the 176-case ambient bench) stays byte-identical to baseline — `fluid-blank-ambient/fused-bench.ts` holds at 175/176. Reformatting validated on cerebras gpt-oss-120b via the new `app-steer-smoke.ts`; it's a best-effort, model-dependent transform (the disambiguation half is the model-robust win). Full design + threat-model notes: `docs/architecture/ambient-context.md` § *App-aware output steering*.
