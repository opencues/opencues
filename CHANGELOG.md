...
No new runtime machinery — this reuses the WIPE splice fluid-blank results have always been able to request via `spanStart`/`spanEnd` (drift guard, chain/undo bookkeeping and the `question`-as-`alt[1]` capture all already existed). The feature is a decision about which range a result covers, plumbed the same way feature #50's char budget is: new optional `HostAdapter.getAnswerReplacesQuery()` (dynamic per current target) → resolver stamps `CueContext.answerReplacesQuery` (no scalar gate — a host-computed boolean about its own element) → `FluidBlankSource` emits the span from the new `replaceQuerySpan()`. mac side: `replaceQueryForBundle` / `DEFAULT_REPLACE_QUERY_BUNDLES` (`com.apple.Spotlight`) in `src/ax-host.ts`, whitelist-overridable via `OPENCUES_AX_REPLACE_QUERY="com.raycast.macos"` (or `"off"`), reached through a new platform-free `DaemonCore.getAnswerReplacesQuery()` accessor.

Two things the wipe deliberately does NOT trust. The span is the runtime's (whole buffer), never the model's `SPAN:`/`MODE:` lines — splice bounds for a slot-splice source must not come from an LLM-claimed span (`docs/architecture/blank-sources.md`). And because hosts in the universal/no-cycling band have no Ctrl+Alt+Down to revert with, `replaceQuerySpan` refuses two shapes even when the host says yes: a multi-line buffer (a newline means it's the user's own content) and a non-trailing `_` (mid-sentence, filling the gap is right whether the model returns the value or the whole clause; wiping is right for only one). Error substitutes (`[OpenCues: …]`) always fill, never wipe — losing a typed question to a transient provider failure would be unrecoverable.
...
### Added — Windows phase 2 (EXPERIMENTAL): word-cues + cycling + click-through overlay + real caret (`@opencues/windows` 0.1.1 → 0.2.0, `@opencues/runtime` 0.18.6 → 0.18.7)
...
### Changed — `@opencues/core` 0.34.0 → 0.35.0 (merge version-collision resolution)

Master's #340 shipped core at 0.34.0; this branch had independently resolved an earlier merge to 0.34.0 with different content. Bumped to 0.35.0 so one version string identifies one content set (and so `lint-version-bump` sees a real bump for the 11 core `src/` files that differ from master). No source change beyond the merge itself.
...
### Added — mac: platform-free daemon-core test suite — the mac host's tests now run on ANY OS, including the maintainer's Linux (`@opencues/mac` 0.3.1 → 0.4.0)

The maintainer has no mac, so the mac integration needed a test surface that always runs. The daemon's entire bridge-event grammar (ready/focus/blur/change/cursor/writeAck + the write path) is extracted from `daemon.ts` into a platform-free `src/daemon-core.ts` — injected bridge-stdin/log/deny-list/env/exit, a STRUCTURAL `RuntimeSurface` interface instead of an `@opencues/runtime` import — with `daemon.ts` reduced to process glue (spawn bridge, wire stdio, boot). `daemon-core.test.ts` (13 scenario tests, vitest, no AX / no swiftc / no darwin API) replays real bridge transcripts from the SPOTLIGHT-SPIKE live captures: the ready handshake (incl. untrusted → exit path), focus-baseline (pre-existing `_` must not arm), deny-list, the full type→arm→fill→echo journey, writeAck failure resync, per-focus write-path logging, the 2026-07-21 Spotlight duplicate-notification REGRESSION (3 identical changes → exactly one arm + one user notify; echo exemption; cursor-only moves not swallowed), the restored-query Spotlight session transcript, char-budget accessor, and line-protocol robustness (malformed JSON, unknown events, pre-attach events). Build is now CI-safe on Linux too: `swiftc` moved out of the package build script into darwin-gated `scripts/build-bridge.cjs` (loud no-op elsewhere — AppKit can't compile off-mac) and the `"os": ["darwin"]` field dropped so pnpm/turbo never skip the package (`opencues install mac` keeps the real platform gate). Note: CI hasn't triggered on the feature branch since July 7 — the suite lands ready for whenever runs resume.

### Fixed — Spotlight double-substitution + countries blank substituting its own error text (`@opencues/runtime` → rolled into 0.26.0, `@opencues/mac` 0.3.0 → 0.3.1)

Live Spotlight report (2026-07-21): `capital of istanbul _` produced `istanbul: not foundistanbul: not found`. Two independent bugs. (1) **Doubling**: Spotlight fires 2–3 byte-identical `AXValueChanged` per keystroke (known since SPOTLIGHT-SPIKE.md); a duplicate non-echo change event re-ran the whole BlankFill pipeline within 2ms (log shows two `deterministic match` + two `substituting` lines) and the second splice landed adjacent to the first. New `shouldDropDuplicateChange()` in `integrations/mac/src/ax-host.ts` drops a change whose value AND cursor already match daemon state — with an explicit echo exemption (the optimistic update in `requestWrite` makes our own write echoes look like duplicates, but the runtime still needs their `'runtime'`-sourced notify for span bookkeeping). (2) **Error text substituted as data**: `CountriesBlank` returned `"<country>: not found"` as a plain string, which BlankFill substituted as if it were the answer. Misses now return the `[err]` feedback sentinel (`[err] "istanbul" is not a country I know`) — same convention as the note/sentinel blanks: fills only the `_`, keeps the typed command editable, never consumes the command span (now the fallback when validation can't decide — see the next entry for the real fix).

### Added — shape-match arg validation: an offline blank's miss RELEASES the `_` to the LLM instead of substituting an error (`@opencues/core` → 0.32.0, `@opencues/runtime` → 0.26.0 — post-master-merge lineage)

The second half of the `capital of istanbul` report: the user rightly expects the AI to answer the trick question ("Istanbul is a city, not a country"), but fluid-blank structurally cedes any `_` claimed by a blank keyword/shape, so after a countries miss NOTHING was allowed to answer. New optional `Blank.validArg?(arg): boolean` capability (synchronous, pure — countries implements it as an offline-table membership check with the same skip-word normalisation as its lookup) → harvested by `boot-common.collectArgValidators` from the host's blank registry → stamped by ConfigLoader onto parsed blank configs as `argValidator` (at READ time via the `blanks` / new `blankOverridesWithValidators` accessors, so hot-reload can't lose it) → consumed at the ONE chokepoint every claim/cede site funnels through: `matchBlankShape` refuses a shape whose captured arg fails validation. Because `blankClaimsUnderscore` (fluid/transform/config-intent cede) and `BlankFill.scan` both call it, the miss releases the `_` EVERYWHERE at once — `capital of istanbul _` is never claimed by countries, fluid-blank answers via LLM (with the Spotlight char budget keeping it short), while `capital of france _` stays deterministic/offline (0 LLM). Resolver now hands sources the validator-stamped view (`blankOverridesWithValidators`) instead of raw `folderConfigs.blankOverrides` — reading the raw record would silently drop validation on the cede path only. All six band boots pass their registry (`BuildSharedRuntimeOptions.blanks`); a band that omits it just keeps the `[err]` fallback. NOT a spec change (`argValidator` is runtime-injected, never parsed from frontmatter; conforming readers unaffected). Pinned by 5 new `blank-shapes.test.ts` cases + `countries.test.ts` validArg tests.

### Added — answer char budget: hosts declare a soft answer-length aim for narrow fields; mac sends 37 for Spotlight (`@opencues/core` / `@opencues/runtime` → rolled into 0.32.0 / 0.26.0, `@opencues/mac` 0.2.0 → 0.3.0)

Spotlight's search field shows ~37 characters, so a prose-length lookup answer scrolls out of view. New optional `HostAdapter.getAnswerCharBudget?(): number | null` (dynamic per current target, like `supportsCycling`) → resolver stamps `CueContext.answerCharBudget` (NO scalar gate — unlike ambient context nothing user- or page-controlled rides the channel, it's a host-computed number) → FluidBlankSource + TransformBlankSource append a shared `renderCharBudgetBlock()` FIELD LIMIT instruction ("prefer the shortest correct answer that fits; exceed only when a correct answer cannot fit") to their fused calls. **A soft aim, never a truncation** — the runtime never cuts model output. Two structural contracts pinned by `packages/opencues-core/src/sources/answer-char-budget.test.ts`: (1) the block rides the **USER message only** — per-call context must never salt the cerebras cached system prefix (`docs/architecture/cerebras.md`); (2) **absent by default** — with no budget the prompts are byte-identical to the pre-feature shape, so the fluid-blank/transform-blank bench evidence stays valid without re-runs (benches never set a budget; `FUSED_SYSTEM_PROMPT`/`FUSED_SYSTEM` untouched). Wired through the universal band (`UniversalBindings.getAnswerCharBudget` passthrough) and the mac daemon: `charBudgetForBundle()` in `ax-host.ts` maps `com.apple.Spotlight` → 37 (measured 2026-07-21), extensible/overridable via `OPENCUES_AX_CHAR_BUDGET="bundle=N,…"` (N < 1 removes an entry — the opt-out). Other bands unaffected (hook absent → `undefined` → structural no-op). Chrome's natural future source is the focused input's `maxlength`/visible width — not wired yet. User-facing summary: `docs/features/answer-char-budget.md` (feature #50).

### Added — `mac`: Spotlight support via panel-agent observers (`@opencues/mac` 0.1.0 → 0.2.0)

`_` blanks now work in Spotlight's search field. Spotlight.app is `LSUIElement=1` with a NON-ACTIVATING panel — Cmd+Space moves key focus to its process without any app activation, so the bridge's single attach trigger (`NSWorkspace.didActivateApplicationNotification`) was structurally blind to it: no observer ever landed on the Spotlight pid and the daemon never saw the field (verified live: panel opened, focused, and accepted typing with ZERO activation notifications). `ax-bridge.swift` now arms a PERSISTENT secondary `AXObserver` per **panel agent** (Spotlight built in; extend via `OPENCUES_AX_PANEL_AGENTS=<bundle,...>` once a new panel app is probed — Raycast/Alfred share the shape) at bridge start and on relaunch (`didLaunchApplicationNotification`; dropped on termination). Arbitration: a text-element focus event from a panel agent wins (it holds real key focus; primary-app focus churn can't steal while a panel owns focus); the panel's release signal is `AXApplicationHidden`/element-destroyed **on the app element** — the field element alone gives NO dismissal signal — which falls back to re-resolving the primary observer's focused element, correct precisely because the primary observer never moved during the panel session. `refocus` became pid-aware end-to-end so the `focus` event carries the PANEL'S app/bundle (the daemon's deny-list checks bundle — mislabeling would have mis-routed deny decisions). Push-only/no-polling design preserved. Spotlight's field measured live: `AXTextField`/`AXSearchField`, honours the atomic `AXReplaceRangeWithText` path (verified by re-read), per-keystroke value+cursor notifications (2–3× duplicates per keystroke — harmless, `freshMarkerAtCursor` compares marker counts), reopens with the previous query fully selected (handled by the existing focus baseline), first Esc clears / second dismisses. Contract-tested end-to-end against live Spotlight (focus event → whole-value replace `method:replace-attr` → echo change events → dismissal fallback). Spike evidence + probe tool: `integrations/mac/SPOTLIGHT-SPIKE.md` + `scripts/spotlight-probe.swift` (5 modes; `auto` is self-contained — opens the panel itself via CGEvent, writes, verifies, types, dismisses).

### Changed — blank-loading custom animation cap raised 5 → 7 frames (`@opencues/runtime` 0.14.0 → 0.14.1)

`CUSTOM_FRAMES_MAX` (caps `blank-loading-frames` AND the parallel `blank-loading-colors-rgb`/`-ansi` lists) raised from 5 to 7 — matching the longest built-in sequence (braille-rotate, 7 frames), so a user-supplied progression can be as long as the shipped ones. Previously a 6+-frame custom list was silently truncated to its first 5 frames. Shipped-defaults + feature-doc comments updated to say "up to 7".

### Added — `mac`: the universal macOS host — `_` blanks answered in the focused text element of ANY app via the Accessibility API (`@opencues/mac` 0.1.0 new; `@opencues/core` 0.18.0 → 0.19.0)

New self-owned host (`opencues install mac` / `opencues run mac`, aliases `macos`/`ax`): a persistent Swift bridge (`integrations/mac/ax-bridge.swift`) tracks the FOCUSED text element in whatever app is frontmost via `AXObserver` — focus / per-keystroke change / cursor events pushed as JSON lines, zero polling — and a thin Node daemon hosts the runtime against it on the universal/no-cycling profile (the `adapters/universal/v1` band, shared with apple-notes — each daemon declares `hostName` for `on-host:` routing). The buffer IS the element's value and the cursor IS the real caret, so host state reduces to three unit-tested pieces (`utf16Diff` single-range writes, `freshMarkerAtCursor` typed-`_` arming — focus content never triggers, `WriteRing` echo classification). Writes are ~1ms in-place range replaces with TWO methods learned per focused element by verifying re-read: the ATOMIC selection-free `AXReplaceRangeWithText` parameterized attribute (private encoding reverse-engineered: `{"AXReplacementRange": <range>, "AXReplacementText": <string>}`; its return value LIES — AppKit's NSObject default stub returns YES without editing) with a select→replace→restore-selection transaction as fallback (three synchronous main-thread calls — never painted, caret handed back delta-adjusted). Rich formatting survives both paths (the app's own text machinery performs the edit). Secure fields (`AXSecureTextField`) never leave the bridge; terminals are denied by default (`OPENCUES_AX_DENY` extends); >200k-unit values blur. Requires the Accessibility TCC grant (distinct from Automation) — probed at install time via `ax-bridge probe`. Verified live end-to-end in TextEdit (typed cue → armed ~250ms → answer in place 1.2s, `write path: replace-attr`). Channel evidence + reverse-engineering notes: `integrations/mac/AX-SPIKE.md`.

### Added — Apple Notes integration: `_` blanks answered inline in Notes.app on macOS (`@opencues/apple-notes` 0.1.0 new; `@opencues/core` 0.17.0 → 0.18.0; `@opencues/runtime` 0.13.3 → 0.14.0; `opencues` CLI → 0.2.55 (post-master-merge lineage))

New self-owned host (`opencues install apple-notes` / `opencues run apple-notes`, aliases `notes`/`applenotes`): a Node daemon polls Notes.app over JXA — one bulk id+modificationDate enumeration per tick (~90ms at 335 notes, measured in `integrations/apple-notes/NOTES-PLATFORM.md`), plaintext fetched only for changed notes, adaptive cadence matched to the standard hosts (500ms active ≈ the resolver debounce, 2s idle = the hot-reload standard, 10s while Notes isn't running). Any unlocked note containing a standalone `_` becomes a fill candidate; the most recently modified one is the runtime buffer. First host on the **universal/no-cycling profile** with zero key/cursor/render channel (`supportsCycling()`/`supportsAgentRewrite()` both false; new band, now at `packages/opencues-runtime/adapters/universal/v1/` since the `mac` host joined the profile; Notes-specific HTML-splice helpers live in `adapters/apple-notes/v1/html-text.ts`). Write-back is deliberately conservative because the daemon edits notes it doesn't own: answers land as a targeted `<div>`-level HTML splice (`spliceLinesIntoBody` aborts on zero or ambiguous matches rather than ever rebuilding a body from plaintext; text-unchanged anchor lines keep their original fragments so heading/bold styling survives adjacent-line fills), every write is compare-and-swap-verified inside a single osascript invocation (iCloud race window ~150ms), attachment-bearing and >30k-char notes are skipped outright, and the daemon's own writes are echo-suppressed by per-note content hash (modificationDate is 1s-resolution and useless for this). Notes' semicolon-less entity serialization (`&lt` not `&lt;` on read-back) is normalized in the band's `html-text.ts`. `opencues doctor` gained an Apple Notes section that probes the Automation TCC permission and documents the silent-deny trap (a cached deny fails in ~70ms with error -1743 and **no dialog** — recovery: System Settings → Automation, or `tccutil reset AppleEvents`). `on-host:` schemas, host-compat `HOSTS`/`NATIVE_HOSTS`/aliases, install/uninstall/update/version/help/completion/which, version markers, and the `--all` install set (macOS-gated) are all wired. Keys arrive via the standard `buildBootApiKeys` boot bag (host → process.env → `~/.cues/.env`) with the zero-key subscription-CLI rung; kata is **deliberately excluded** (no key channel for `observeKey`, no statusline — see `docs/architecture/kata.md`). **Security:** this is the first host whose buffer is remotely writable (shared notes / iCloud edits from other devices are indistinguishable from local typing) — documented as `docs/architecture/security-audit.md` row #30 (🟡; shared-note skip / folder allow-list tracked as a follow-up needing JXA verification on a real Mac). **Reliability (2026-07-12):** Notes.app periodically wedges its Apple Events queue (ledger rows 25-26 in `integrations/apple-notes/notes.md`) — the daemon now detects it (≥2 bridge timeouts in 60s via a central `onTimeout` hook on every osascript call) and AUTO-RESTARTS Notes itself (AppleScript quit → SIGTERM → SIGKILL escalation, reopen with `open -g`), gated by a 5-min anti-thrash cooldown, 2s FSEvents quiescence (never mid-keystroke), and a drained fill chain (never severs a CAS); a user-quit Notes is never relaunched. Verified live against a SIGSTOP-simulated wedge: detected at 21s, Notes restarted, polling healthy 8s later. Recently-Deleted notes are now excluded AT THE SOURCE — `list-notes.js` subtracts every account's Recently Deleted folder inside the same osascript call and returns live notes only (`deleted-ids.js` and its 10s exclusion refresh deleted; the 10s window where a just-deleted note kept competing for the active buffer is structurally gone; the changed-while-deleted visibility warn survives on the same call's `deleted` payload).
...
### Fixed — five `opencues` CLI command bugs surfaced by the phase-3 coverage pass (`opencues` CLI → 0.2.55 (post-master-merge lineage))
