...
### Added — Firefox MV3 add-on (`@opencues/firefox` 0.1.0, CLI 0.2.56)

A new Firefox integration at `integrations/firefox/` with feature-parity to the Chrome extension — cues, blanks, cycling, dim rendering, popup, and full native-messaging-host support (live `~/.cues/` sync + scripted-blank exec). It's a fork of the Chrome integration's build/manifest/install/`src/` layer, reusing everything below the browser boundary: `@opencues/core`, `@opencues/runtime`, the chrome v1 runtime adapter (Firefox reports `hostName: 'chrome'`, so `on-host: chrome` configs work unchanged), and the native host script (`integrations/chrome/host/host.cjs`, reused verbatim — not forked). Firefox deltas: `browser.*` namespace (promise API; `chrome.*` alias is callback-only), MV3 event-page background (`background.scripts` + `type: module`, not `service_worker`), `browser_specific_settings.gecko.id` (default `opencues@opencues.dev`) + `strict_min_version: 140.0` (CSS Custom Highlight API), and native-host registration with `allowed_extensions` under the Mozilla `NativeMessagingHosts` dirs / `HKCU\Software\Mozilla` registry. CLI wiring: `opencues install firefox` / `install firefox-host` / `sync firefox` / `uninstall firefox` / `run firefox`, plus a doctor section and drift-detection marker. `firefox` is kept out of `@opencues/core`'s `HOSTS` (CLI-only target) so host-compat semantics stay unchanged. Load via `about:debugging` → Load Temporary Add-on. Known gaps (tracked follow-ups): `src/` is a fork that will drift from Chrome's; the unit/e2e suites and a signed `.xpi` are not yet ported. See `integrations/firefox/{README,CLAUDE}.md`.
