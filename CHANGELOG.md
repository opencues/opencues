...
The location `map` card (name / full address / Google Maps link), the claude-status block, the `model` catalog, and the note recall are each a SINGLE multi-line answer, but `BlankFill.applyAsyncFill` split every multi-line `get` result on `\n` into cycleable alternatives and committed only line[0]. So `map east finchley station _` delivered just `East Finchley Station` and silently dropped the address + the map URL (reported on macOS as opencues #339). That split is correct for genuine list blanks (a top-N feed like hackernews, whose lines ARE separate choices), so the fix is a per-blank opt-in rather than a global change: a new `blankMultilineIsAnswer: true` frontmatter key (default `false`) makes the fill join the lines into one answer instead of truncating. Added to the four single-card default blanks (`location`, `claude-status`, `model`, `note`); hackernews and other list blanks keep per-line cycling by omitting it. New spec frontmatter key (§ Frontmatter optional + § Flag obligations in `blank-spec.md`, `blank.schema.json`, `SPEC_VERSION 0.8 → 0.9` — additive, `0.8` readers ignore the unknown key). Pinned by two `blank-fill.test.ts` cases (with-flag joins, without-flag still splits) + agentic scenario 112 (`map _` end-to-end retains the `Map:` URL line). Note: existing user installs with a stale `~/.cues/blanks/location/BLANK.md` need a re-seed (`opencues seed-configs`) to pick up both the `map` keyword and the new flag.
