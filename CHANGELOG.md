...
### Fixed — chrome: a second `_` in a non-cycling `<input>`/`<textarea>` was silently ignored (`@opencues/runtime` 0.28.2 → 0.28.3, `@opencues/chrome` 0.2.90 → 0.2.91)

On a plain (non-cycling) chrome field — Luma's form fields, any normal `<input>`/`<textarea>` — the FIRST `_` filled correctly but every subsequent `_` was silently dropped with no log line. Root cause: after a fill, the runtime's own view of the buffer went stale. A normal-input fill goes through `writeNormalInputValue`, which dispatches a synthetic `input` event with `isTrusted=false`; content.ts drops those for security, so the fill never round-trips back to the runtime. And `runtimeRender()` early-returns for normal inputs (they can't host CSS Custom Highlights), so the render path that also refreshes the buffer baseline never fired either. With both refresh paths skipped, the chrome adapter's `lastSeenText` froze at the pre-fill value (the first `_`). The resolver's explicit-`_` gate reads that as `previousText`, so the next `_` computed `blankJustTyped = endsWith('_') && !prevEndsWith('_')` → false and the count-diff fallback `newU > prevU` → false, and the trigger was discarded before the gate's own BLOCKED log — hence completely silent.

This is the "stacked-blank after substitute silently no-ops" class (documented at `resolver.ts` § explicit-`_` gate): the `e.previousText` gate is correct, but it relies on the adapter keeping `lastSeenText` current, which the normal-input write path didn't. Fix: wrap `setText` in the chrome adapter band (`adapters/chrome/v1/boot.ts`) so every runtime write updates `lastSeenText`/`lastSeenCursor` — a pure variable write, no event emit, no re-entrancy, covering every host write path (cycling or not). Managed-editor writes already round-tripped a trusted `input` event, so this only changes the previously-broken normal-input case. Verified live on Luma: repeated `_` in the same form field now fills every time.
