...
- **Per-sentence call cache + in-flight sharing.** Every pass re-ran every sentence-scope cue on every sentence and aborted the previous pass's calls; on a multi-sentence draft that re-sent every unchanged sentence (34% of sentence calls in the Sep 2026 log) and threw away calls for finished sentences (every logged contradiction parse). `SentenceCallCache`, one per source, keyed on the exact LLM input: a completed answer is reused (positive, empty and ceded; never an error), and a superseding pass joins the in-flight call for an unchanged sentence instead of aborting it. Used by `SentenceCueSource` and `ContradictionLlmSource`. A three-sentence draft typed through the opencode harness under shipped defaults: 40 calls where the fan-out alone would have been about 70.
- **Shape-aware pause.** `Resolver.pickDelay`: a closed sentence (terminator or newline) fires after 100ms instead of 500, a pause inside a word of a space-delimited script waits 800ms, everything else keeps 500; a `.` after a digit is not a terminator; a whitespace-only append schedules nothing, so the space after `party!` no longer fires a twin resolve 160ms after the fast one. Structural signals only, so it holds in any script (a CJK letter is never "inside a word"). Options `terminatorDebounceMs` / `inWordDebounceMs` on `ResolverOptions`. Measured: fire counts on single-sentence drafts unchanged at 70 wpm (the wasted fires sit after spaces, not inside words); every sentence end gains half a second.
...
### Fixed — a tip's rewrite cannot smuggle a launch flag or a keybind into the prompt; the Gemini CLI pack's `when:` lines reviewed on both models (`@opencues/core` 0.60.2)
- `isGroundedRewrite` accepted `i want to work on the hotfix … --worktree` and `Press Ctrl+R to reverse-search for <draft>` because they keep every word of the draft. Both are advice to the person, not a prompt: a rewrite that adds a launch flag (`-p`, `--sandbox`) or a keybind (`Ctrl+R`, `Shift+Tab`, `Esc`, `F12`) the draft did not have now collapses to an advisory. `@path` and `!cmd` stay, since a prompt can carry them, but an `@path` the rewrite adds must be a path the draft named: `<draft> @path/to/screenshot.png` is a placeholder the person would have to edit, so the `screenshot` tips are advisories. Seen live on gpt-oss-120b for `--sandbox`, `--worktree`, `Ctrl+R` and `Ctrl+T`.
- Gemini CLI pack: eight `when:` lines rewritten after a 101-phrase probe pass (two phrasings per entry plus 35 topic-adjacent traps) on gpt-oss-120b and qwen-3.8-27b. `/auth` no longer claims every draft that mentions an API key, `/tools` a toolbar, `Ctrl+R` a search feature; `--sandbox`, `/extensions`, `/agents`, `--resume` name the synonyms people type (container, plugin, subagent, an earlier session). qwen recall on the probe set 59 → 62 of 66, still zero false alarms; gpt-oss flat. The shipped bench gains ten recall cases and six traps that held on both models (24 / 12).
- The bench takes `--pack-file <path>` to A/B a rewritten pack against the shipped one. Finding recorded in the architecture doc: on gpt-oss a single changed `when:` line moves verdicts on unrelated phrasings (one line flipped four cases in the probe set), so a per-line probe delta of a few cases is cross-talk, not signal; the shipped gate on both models is what a pack edit has to pass.
...
- **Packs are situations.** Every entry in the four shipped `tips-*` packs has a `when:` line (the situation, in the person's words) and a `say:` line (the advice, naming the command); the `alts` sibling lists are gone. Claude Code 44, OpenCode 37, Gemini CLI 41, shell 15 (were 129 / 96 / 134 / 62 static keys). A `screenshot` situation added for Claude Code.
- **One call per pause.** `SemanticTipsSource` (priority 86, the tips leg of the fused session rail: contradiction → tips → ask) matches the whole draft against the catalogue in the system message (prefix-cached). Three grounding rules: the cited id must exist, the quoted clause must be a verbatim substring of the buffer, and the solution must be the entry's own slash command (`entryCommand`: the name when it is a command, else the first command the `say:` line names; a launch flag is never one) or a rewrite that keeps half the person's words and is not the pack's own text (`isGroundedRewrite`). Anything else collapses to the command or drops to an advisory. The note says the pack's `say:` line; the model's "why" goes to metadata.
- **`_` is the solution.** A command tip spans the whole draft and `_` swaps it for the command, `_` again reverts. A prose tip swaps the flagged clause for the grounded rewrite. An advisory has nothing to swap: `_` mutes it, twice within three seconds forgets it. One emoji leads the note (`splitNoteEmoji` takes any leading pictographic glyph); the hint is a verb (`apply` / `revert` / `dismiss`), two-stop notes are toggles that read `was: <snippet>` after the press.
- **`tips-mode: semantic | off`** (`semantic` default). `on` and the short-lived `definitions` read as `semantic`; `opencues seed-configs` rewrites them. `tips-shard-size` (`50` default, `35`, `20`, `off`): a catalogue past the ceiling is cut on section boundaries into balanced shards, one parallel call each (`shardCatalog`, `matchShard`), flags merged in shard order — a small model abstains as one list grows (qwen-3.8-27b: 44 lines 18/20, 122 lines 15/20; sharded 18/20 at the same latency). Every shipped pack stays one call.
- **The static layer is removed.** No cue map in `ConfigLoader` (`lookup` returns blank tips only; `navigableWords` is blank keywords only); no gray, navigation stop, status-line definition, caret note or sibling cycling on a typed pack word; the resolver's curated-list guard, the static def constructors and the cycling fallback that materialised a def from a typed word are gone; TTS reads a def's own `cueTip` (the per-word static `speak:` hint is gone). `@opencues/core` drops `LocalCueSource`, `buildLookupMap`, `lookupMultiple`, `lookupWord(s)`. Only registered DynDefs gray and cycle: an LLM word-cue, a spelling fix, a sentence cue, a filled blank.
- **Two fixes the removal surfaced.** Navigation's empty-config scaffold fallback counts a tips catalogue as config content. A sentence cue anchored on a word that already carries a word-cue def registers over it (the resolver's "already resolved" guard is exempted for sentence cues; the same cue re-resolved is a refresh, never a second def, and a refresh grows the span with the draft).
- **A failed undo no longer eats the draft.** When the fluid-config classifier reads a whole draft as an undo of OpenCues' own change and nothing applies, the `[OpenCues: could not undo]` note replaces only the command tail before the `_` (or just the `_`), never the prose.
- **Spec `0.12`.** `cue-spec.md`'s "Mode 1: static" becomes the situation catalogue (`tip` / `when` / `say` / `emoji`; `alts` removed and MUST be ignored; no token matching; "combined mode" goes); `tips-mode` values; schemas and conformance fixtures updated. `spec/CHANGELOG.md`.
- **Bench.** `tests/benchmarks/tips/semantic-tips-bench.mjs --pack <host> [--model m] [--shard n] [--stack a,b] [--probe "<phrase>"]`: per-pack recall + trap sets and a solution column; gate = 0 false alarms, ≥ 90% cited right, every command case lands its command. Met on all four packs on cerebras gpt-oss-120b and qwen-3.8-27b (`docs/features/semantic-tips.md` § Where it stands). `--probe` runs any phrase on any model; terse phrasings are the small model's ceiling.
- **Tests.** The cycling / navigation / dim corpus that drove its mechanics through static word alternatives seeds the same defs explicitly (`testing/mock-adapter.ts: seedWordDefs`). `docs/features/local-cues.md` retired. Docs: `docs/architecture/semantic-tips.md`, `docs/features/semantic-tips.md`, `docs/features/note-hints.md`.

### Added — fluid-config reaches every `config _` menu entry: MENU_TUNABLES join the classifier, `claude` provider alias (`@opencues/core` 0.57.0 → 0.58.0)

A coverage sweep of the natural-language settings classifier (every value of every setting on Claude Code, four varied phrasings each, 248 asks) found 6 of the 35 entries the `config _` menu cycles were unreachable by sentence — the MENU_TUNABLES (`blank-loading-animation`, `blank-loading-interval-ms`, `glimmer-transition-ms`, `agent-debounce-ms`, `blank-context-prewarm-ms`, `max-concurrent-auditors`; `dim-mix` on chrome/dsh). The v1 deferral was about free numeric codomains, but every tunable declares the same closed PRESET list the menu cycles, so they are as bounded as a feature enum. `buildFeatureBlock`, the recall prefilter and `validateAgainstRegistry` now read FEATURES + MENU_TUNABLES (host-scoped alike); the classifier may only name a listed preset (`set the agent debounce to 500ms _` works, `wait 7 seconds _` is NONE). Two few-shots added: the terse `tips on _` toggle (a bare `<setting> on _` was rejected while `<setting> off _` passed) and a tunable example. `parseConfigIntentOutput` normalises model-emitted provider aliases (`claude` → anthropic, `google` → gemini) before validation — `blanks on claude please _` previously died as "unknown provider 'claude'".

Bench: `tests/benchmarks/fluid-config/coverage.ts` is the new full-registry sweep (hits + rejects, per-setting table); `cases.ts` / `cases-holdout.ts` had six expectations still naming `user-context-mode` (renamed to `identity-context-mode` in June) — the model was answering the current name and being marked wrong, understating recall by ~12pp; the bench's 128-token cap also truncated ~5% of verdicts mid-line on gpt-oss (the runtime floors that call at 2048) — raised to 512. The sweep also mirrored the runtime's deterministic undo/redo pre-pass into the bench, which exposed a real bug: `enable undo _` PERFORMED an undo (the trailing-alias string match) instead of flipping `undo-mode` — the matcher now cedes to the LLM when a settings verb or question stem precedes the alias (`enable undo _`, `turn on undo _`, `how do i undo _`) and accepts a trailing demonstrative (`redo that _`). Same-session on cerebras gpt-oss-120b, before → after: in-prompt settings 100% / 84.8% → 100% / 95.0% (P / R), undo 95% → 100%, holdout 100% / 78.9% → 100% / 89.5%, coverage sweep 88.9% → 95.7% hits at 100% rejects (262 cases); live on Claude Code 20/20 (`tests/benchmarks/fluid-config/EXPERIMENTS.md` § 2026-09-05). `@opencues/dsh` 0.2.19 → 0.2.20: inline bundle regenerated with core 0.58.0.

### Fixed — reverting a `change X to Y _` transform (Ctrl+Alt+Down) corrupted the buffer when the edit wasn't the first word (`@opencues/runtime` 0.39.1 → 0.39.2)

A transform-blank substitute whose edit landed via the replace-parse bounded-splice path (a single verified substring edit, e.g. `the boy ran fast change boy to girl _` → `the girl ran fast`, shipped on by default) registered its revert DynDef at the WRONG word. The def always spans the whole rewritten body from char 0 (both `transformDef` constructors hardcode `spanStart: 0`), but its DynDefs map KEY — the origin index that `findSpanContaining` / navigation / cycling redirect the whole span through — was derived from `spliceStart`, i.e. the EDITED word's offset. When the edited target wasn't the first word ("girl" at char 4, word 1), the def was parked at word 1 while its span still began at word 0. `findSpanContaining` then reported origin 1, the whole-span redirect never fired, and Ctrl+Alt+Down spliced the entire original in place of the single word "girl" — corrupting the buffer to `the the boy ran fast change boy to girl _ ran fast`. The whole-buffer three-way-merge path was unaffected (there `spliceStart` is 0, so the origin and edited words coincide), which is why the fused-path scenarios never caught it. Fix: key the def at its span-origin word (the word covering `spanStart`, char 0) instead of the edited word — byte-identical on the merge path, correct on the bounded-splice path. Glimmer still points at the changed region via its own diff. Found by the agentic harness (scenario 44); pinned by two new `transform-blank.scenarios.test.ts` cases (the def-key invariant + the full Down-revert / Up-restore round-trip), both red before the fix.

### Fixed — caret-in-span highlight now covers selector-satellite pairs and filled blanks, not just transform/fluid spans (`@opencues/runtime` 0.39.0 → 0.39.1)

Moving the caret into `config _`'s landed pair or `claude status _`'s filled span switched the inline NOTE correctly (the note branches are cursor-aware) but the paint stayed DIM — only DynDefs-managed spans (transform/fluid/cues) had the dim→highlight auto-select promotion, so the "you're on it, `_` engages it" affordance was missing for exactly the elements whose notes invited engagement. Identical on every host (shared DimRender logic). The spanFill and selector-satellite note branches now set `cursorSpanLogical` like the DynDefs branch always has: caret on the SELECTOR highlights the setting name (value stays dim), caret on the SATELLITE highlights the value (name stays dim) — mirroring the note's own part-awareness — and a filled list/script blank's span promotes whole. One deliberate supersession: a whole-buffer consume-all fill now promotes under the caret too, matching the transform-blank whole-buffer precedent (the old "stays dim at cursor 0" pin was the inconsistency itself); a caret-less ctx (cursor < 0) keeps the contiguous dim. Pinned by the extended spanFill/selector-satellite cases in `dim-render.test.ts`; verified live on CC and OpenCode via the bridge (highlight `{0,14}` on the selector at cursor 2, `{15,18}` on the value at cursor 16, byte-identical across hosts).

### Fixed — `claude status _` cycles its four answers again instead of pasting a four-line card (`@opencues/runtime` 0.38.5 → 0.39.0, `opencues` CLI 0.7.10 → 0.7.11, `@opencues/chrome` 0.2.200 → 0.2.201, `@opencues/dsh` 0.2.18 → 0.2.19)
...
### Fixed — loading-frame colour flashed on the substituted text at the moment of resolution (`@opencues/runtime` 0.38.4 → 0.38.5)

`config _` → at the instant the satellite landed, the LAST word flashed red — identically on CC and chrome, because the emitter is shared (`boot-common`'s loading render handler; red is frame 0 of the shipped palette: ANSI `red` on terminals, `#ef4444` on chrome). Root cause: `getActiveColoredRanges` painted the slot's **word index** with the frame colour without checking the word was still a loading glyph. In the render(s) between a substitution landing and the owner's `stop()` arriving, the slot is still active but its index addresses the SUBSTITUTED text — `config _`'s slot (index 1, the `_`) suddenly pointed at `on` of `word-cues-mode on`. Fix: the emitter now consults `isOurSlotChar` — the same staleness predicate BlankFill's `applyAsyncFill` has used since the owner-refcounting era — so only a word that is still the `_` or a current frame glyph is ever coloured (ZWS render-kick chars stripped before the check). Pinned by the staleness-guard cases in `blank-loading.test.ts`; the coordinate-mapping fixtures were also corrected to animate a genuine `_` slot (the old ones coloured ordinary words the guard rightly refuses).

### Fixed — inline note wider than the terminal vanished to a bare `…`; splice hosts now WRAP like every other platform (`@opencues/runtime` 0.38.3 → 0.38.4)

`config _` → the satellite's inline note (a 78-char setting description at span column 15 needs ~97 cells) painted as nothing but `…` on narrower terminals — on every CC version (A/B'd on 2.1.206 and 2.1.236): the runtime spliced the note as one width-unaware line and CC's Ink box truncates over-wide lines degenerately. Platform parity is the fix's spec: chrome lays the same note out `pre-wrap` in a max-width box and the OpenTUI hosts (OpenCode/shell) render it as a flow `<text>` that wraps and grows the input — the splice hosts (CC/Gemini) were the outlier. `applyDirectives` gains an optional `maxNoteCols` and the note now **wraps into hang-indented continuation lines** within it (`wrapToCellLines` — greedy word wrap, CJK counted at 2 cells, spaceless runs hard-break like `overflow-wrap`; the `(underscore to cycle)` hint wraps inline like chrome's). A span column too deep to leave 12 readable cells loses alignment (pad shifts left) rather than the measure. CC passes its live `stdout.columns − 3` per render (resize-safe); omitted → single unclamped line, byte-for-byte prior behaviour (the bridge's buffer-space `renderedText`). Verified by tmux capture at 80 and 60 columns — the full description now paints across two lines — and pinned by the `maxNoteCols` wrapping + `wrapToCellLines` describe blocks in `render-directives.test.ts`.
