...
### Added — per-target markdown pass-through for hosts without a styling surface (`@opencues/runtime` 0.13.3 → 0.13.4)
...
### Changed — Claude Code pin bumped `2.1.170 → 2.1.206` (`@opencues/claude-code` 0.2.7 → 0.2.8)
...
**Fixed en route — tweakcc system-prompt writeback corrupted the 2.1.206 binary.** tweakcc's `applySystemPrompts` runs unconditionally (before the `patchImplementations` map setup.sh § 4d disables) and re-embeds every extracted system prompt; its backtick re-escaper doubles backslashes inside preserved `${...}` interpolations, and 2.1.206's memory prompt introduced a nested template (`` ${l?`\`${y}\``:y} ``) that corrupted to a parse error — the patched binary died on ANY launch (`SyntaxError ... Expected ':' in ternary operator`), with all OpenCues seams applied correctly. New setup.sh § 4e patches the callsite to pass an empty `patchFilter`, skipping the prompt writeback entirely (OpenCues never customizes system prompts). Post-mortem: `packages/opencues-runtime/adapters/cc/REPAIR.md` § 15; gotcha added to UPGRADING.md.
