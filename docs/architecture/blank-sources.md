...
| **`SemanticTipsSource`** (`semantic-tips-source.ts`) | a settled draft shows a situation on the tips catalogue (`when:` lines) | A grounded quote + the cited entry's slash command (or a clause rewrite) | Passive sentence-cue DynDef; `_` swaps the whole draft for the command. See `docs/architecture/semantic-tips.md`. (Replaced `LocalCueSource`, the static per-word tips path retired in spec 0.12.) |
...
With a decision package installed (`decisions-provider`), the runtime resolver asks the `route`
leg before a `_` pass and dispatches only the routed chat source (config-intent / transform-blank / fluid-blank);
...
rest, so routing never loses an answer; a route past `UNDERSCORE_ROUTE_BUDGET_MS` (1 s) fans out
without tripping the breaker. See `docs/architecture/decisions.md` § Legs.
