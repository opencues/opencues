...
| 31 | Remote-writable buffer — shared-note / cross-device `_` injection (apple-notes) | Every other host's buffer is local keyboard input; the apple-notes daemon's buffer is ANY unlocked note — including **shared notes and iCloud edits synced from other devices**. A collaborator typing `<request> _` into a shared note is indistinguishable from the owner typing it: the synced edit lands as a `source: 'user'` poll change, arms the fresh-marker gate, and triggers LLM calls and keyword-routed blanks (volume/brightness scripts, settings changes via selector-satellite) on the owner's machine. This breaks the standing "buffer = local user input" assumption every other row implicitly relies on. | (a) The reachable surface is exactly the capability-gated one local typing reaches — user blanks require the explicit trust list (#2), script blanks are path-validated + sandboxed (#15-#17), and fluid/transform output lands only as visible note text (no tool/exec channel — same structural invariant as #21). (b) The fresh-marker arm (`freshMarkerIndex` in `integrations/apple-notes/src/tick.ts`) only fires for a marker inside the changed region, so a passive stale note can never fire. (c) The startup baseline (`seedBaseline`) never retro-fires pre-existing cues. (d) Writes are splice-only + CAS-verified, so a hostile concurrent editor can at worst make fills abort, not corrupt. | A collaborator on a shared note CAN deliberately invoke the owner's enabled blanks, change the owner's OpenCues settings via a settings phrase, and burn the owner's LLM quota. The daemon cannot currently distinguish shared notes (JXA `shared`/sharee property support is unverified — needs a real Mac). Mitigation options (folder allow-list scalar, local-account-only scope, shared-note skip) tracked in Open follow-ups. Surface documented in `integrations/apple-notes/README.md`. | 🟡 |

## Open follow-ups

The amber / partially-covered items each have a known next step.

**Apple Notes shared-note scope (#30).** The daemon watches every
unlocked note ("any note" was an explicit product decision, 2026-07-06),
which makes shared notes a remote trigger channel. Next step, on a real
Mac (JXA properties can't be verified from CI/WSL): probe whether
Notes' scripting dictionary exposes a `shared`/sharee signal per note —
if yes, skip shared notes by default (security-first: cut the surface,
not the feature); if no, ship a folder allow-list scalar
(`apple-notes-folders:` in OPENCUES.md) so cautious users can scope the
daemon to notes they own alone. Until one of those lands, the surface
is documentation-mitigated only.
