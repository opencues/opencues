...
router has a 1000 ms budget (`UNDERSCORE_ROUTE_BUDGET_MS`): past it the pass fans out as today and
the breaker stays shut (a 3.6 s route was seen once on a fresh host; a slow router must never hold
the `_`). The provider is built with the sources from the same options, and `decisions-provider` /
...
| blank routing by Choice | keyword windows + shapes | the `_` router's `device` route naming WHICH registered blank (`make it louder _` → volume), grounded in the registered names; ships only through `security-audit.md` row #32 (selection only, arguments by grammar, tiered by code identity + user trust) | the runtime must accept a routed blank; security-audit review (a fuzzy invocation path) |
| batching — **benched 2026-09-17, NEGATIVE, do not re-run** | the rail's request and the sentence gate's request are separate | ONE decision request per pass assembled by the resolver | built (`CueSource.decisionQuestions`, answers on `CueContext`, 9 tests green) and benched with `batch-bench.mts`: the real resolver, the real rail + more-formal gate, 56 passes, two interleaved runs — requests per pass 1.77 → 1.00, chat calls and $ identical, pass p50 457/551 → 524/499 ms, rewritten sentences 512/593 → 584/597 ms: within noise, no gain. The premise was wrong: the resolver already dispatches sources in parallel, so the two requests were already concurrent; merging them makes one bigger, slower request (2531 in / 553 out vs 1525 / 303) that every asking source waits for. Step 4's +190 ms is the gate's own round trip in front of the rewrite call, which no batching removes. Future stacked questions ride the rail's request (as 7B's typo question does) with no resolver change. Not merged; the bench is kept as evidence. |
