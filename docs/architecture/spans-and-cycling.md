...
| CC hands `applyRender` a **scrolled viewport slice** of buffers taller than the input zone, while def/cue spans are full-buffer coords → `defSpanLive` read every scrolled span as stale → tall transform rewrites (`draft email _`) lost dim + note + visible selectability, on every CC version. Invisible to every short-buffer test AND to the bridge's `renderDirectives()` dump hook, which recomputes against the full buffer (a different code path than production's slice-fed render) | `adapters/cc/v2.1/viewport.ts`: locate the slice inside the full buffer (one-space cursor-cell pad handled; ambiguity → occurrence containing the caret), build ctx from FULL text, translate every directive family back to slice coords, clip off-screen; no contiguous match → pre-fix behaviour. Bridge dump gains `lastRender` (the REAL pipeline's last invocation) and DimRender debug-logs a live-def-painted-nothing invariant so the class self-identifies in `/tmp/opencues.log` | `viewport.test.ts` + `boot.test.ts` § viewport-slice rendering (scrolled slice paints; span above viewport paints nothing; fallback never throws) |
| Transform-blank revert (Ctrl+Alt+Down) corrupted the buffer when the edit came via the **replace-parse bounded-splice** path and wasn't the first word (`change boy to girl _` → `the girl ran fast` → revert gave `the the boy ran fast … ran fast`). The def spans the whole body from char 0, but its DynDefs **key** was derived from `spliceStart` (the edited word's offset), so it was parked at word 1 while its span began at word 0 → `findSpanContaining` reported origin 1 → the whole-span redirect never fired → `applyAltCycle` replaced only the edited word's range. Merge path unaffected (`spliceStart==0` there), so only the bounded-splice path (agentic scenario 44) exposed it | Key the transform def at its **span-origin** word (the word covering `spanStart`, char 0) not the edited word (`resolver.ts` transform-substitute branch) | `transform-blank.scenarios.test.ts` — "keys the def at the span-origin word, not the edited word" + "Ctrl+Alt+Down reverts the whole buffer to the original, not a single word" |
...
## Deferred alternatives (core 0.67.0, runtime 0.41.9)
