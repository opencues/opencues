...
| [`user-context.md`](user-context.md) | Optional `user-context-mode` — fluid-blank receives sentinel tokens from `~/.cues/USER.md`. Safe-mode (post-process substitution) vs raw-mode (inline values). PII never reaches provider logs in safe mode. |
