...
| [`undo.md`](undo.md) | `undo _` / `redo _` — the UndoJournal transaction log (diffSplice buffer entries, epochs, coalescing), the ACTION verdict on the config-intent classifier, and the UndoApplier's exact-match-or-refuse policy. Tier-4 (user-pack external effects) documented as out of scope. |
