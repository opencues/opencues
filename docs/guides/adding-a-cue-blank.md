...
| `blankKeywords` | Yes (unless `blankShapes`) | string\|string[] | Friendly shorthand: keywords that bind a `_` to this blank. Desugar at parse time into anchored `blankShapes`. Multi-word phrases allowed. |
| `blankShapes` | No | BlankShape[] | Explicit anchored routing grammar (`{pattern, action: get\|set\|step, valueGroup?}`), matched against the line containing `_`. Wins over `blankKeywords` if both are present. The single low-level routing mechanism; keywords are sugar over it. |
