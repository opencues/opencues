...
> If your `claude` binary is at a non-standard path (e.g. WSL `claude-cues` at `~/.opencues/forks/claude-code/`), pass it explicitly: `pnpm exec opencues install claude-code --target ~/.opencues/forks/claude-code/node_modules/@anthropic-ai/claude-code/cli.js`
