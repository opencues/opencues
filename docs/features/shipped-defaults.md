...
|---|---|---|
| `opencues seed-configs` | On every invocation (standalone or chained from `opencues install <host>`) | Four phases: (1) **SEED** first-time copy of `OPENCUES.md` + `AUDITORS.md` + the `cues/`/`blanks/`/`auditors/`/`scripts/` folders to `~/.cues/` — preserves non-empty user files; (2) **SYNC** library files (`.sh` / `.cs` / `.ps1`) from `defaults/{blanks,scripts}/` every run — overwrites stale, never overwrites `.md`; (2.5) **SHIPPED-MD REFRESH** of `cues/`/`blanks/`/`auditors/` `.md` files: contract frontmatter fields (`on-host`, `sandbox`, `blankScript`, …) always follow defaults, user fields are preserved, and the BODY follows defaults only while it has never been edited — `~/.cues/.shipped-bodies.json` records the shipped body's hash at seed/refresh time, so a tuned shipped tips pack reaches an existing install but a body you rewrote is never touched; (3) **HEAL** re-seed a 0-byte `OPENCUES.md` (the runtime settings file — empty would silently break every runtime-settings read); (4) **COMPILE** colocated `.cs` → `.exe` (WSL only). It never touches `CUES.md`/`BLANKS.md` — those aren't shipped in `defaults/` at all. |
