...
want caught, and with a decision provider (`decisions-provider: typesafe`)
...
unknown …` on that sentence. Benchmarked on twelve multi-sentence drafts next
to the engineering defaults: 8/8 planted hedges and deferrals cited the right
rule and the right sentence, the engineering rules were never cited by
mistake. Two limits worth knowing: one rule per pause (the strongest; the
