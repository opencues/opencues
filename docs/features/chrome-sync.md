...
The native hosts use a two-tier search path — `~/.cues/` (user-level) plus `<cwd>/.cues/` (project-level) — exactly like `.editorconfig` or `.npmrc`. The project you `cd` into determines what the integration sees.
...
That's what happened during sync-demo testing: the watcher got started from `~/testing` and silently ignored all edits to the opencues repo's own `.cues/`. The bug isn't the watcher — it's the cwd inheritance.
...
2. **`~/.cues/`** — the user-level default. Always first unless `$OPENCUES_HOME` is set.
3. **Each `--include <path>`** — added in the order given, stackable, repeatable.
4. **`--project`** — if passed, adds `<cwd>/.cues/`. Highest project priority.
...
| Situation | Command |
|---|---|
| Typical home user — one set of configs in `~/.cues/` | `opencues sync chrome --wsl` |
| Iterating on the opencues repo's own `.cues/` | `opencues sync chrome --include ~/opencues/.cues --wsl --watch` |
| Bundling a project you're currently inside | `opencues sync chrome --project --wsl` |
| Bundling configs from *n* projects at once | `opencues sync chrome --include ~/a/.cues --include ~/b/.cues --wsl` |
| Trying out a single pack end-to-end | `opencues sync chrome --pack demo-pack --wsl` |
| Sandbox test from an arbitrary dir | `opencues sync chrome --source /tmp/.cues --wsl` |
...
- **`sync chrome --wsl --watch`** watches `~/.cues/` only. Edits anywhere else are invisible to chrome.
- **`sync chrome --include ~/opencues/.cues --wsl --watch`** watches `~/.cues/` AND `~/opencues/.cues/`. Edits in either propagate within the debounce window (~250ms).
- **`sync chrome --project --wsl --watch`** watches `~/.cues/` AND `<cwd>/.cues/` — where `<cwd>` is frozen to the directory the watcher was started in. This is the ergonomic footgun: subsequent `cd` moves do nothing.
...
| Role | Lives at | Scope |
|---|---|---|
| **User-level** | `~/.cues/` | "My default cues across every project I work on" |
| **Project-level** | `<cwd>/.cues/` | "This repo's specific cues" — only the native hosts use it automatically |
...
Before this change, `sync chrome` defaulted to user + `<cwd>/.cues/` merged, same as the native hosts. If you had a script or muscle memory that did:
...
# or, for --watch:
opencues sync chrome --include ~/myproj/.cues --wsl --watch
```

The `--user` flag that existed before (explicit "only ~/.cues") is gone — it's now the default, so it had no purpose.
