...
| `tables` | `TablesBlank` | **`table-lookups-mode: on`** (off by default). See [Tables & calculators](tables.md) for the full catalogue. Eight offline tables, no LLM: `unicode for em dash _`, `hex for tomato _`, `http status for not found _`, `mime type for png _`, `default port for postgres _`, `convert 5 miles to km _`, `calc 17 * 23 _`, `atomic number of gold _` (also `atomic mass of`, `boiling point of`, `melting point of`, `ph of`), plus a calculator library (`blanks/calc/`): dates & durations (`days between 3 march and 19 september _`, `90 days from today _`, `last friday of october _`, `in 45 minutes _`, `age if born 14 march 1990 _`, `easter 2027 _`), timezones (`time in tokyo _`, `3pm london in tokyo _`, `meeting at 3pm london for new york tokyo _`, `is it dst in london _`), number formatting and number theory (`1234567 in words _`, `2024 in roman numerals _`, `255 in hex _`, `0.375 as a fraction _`, `round 3.14159 to 2 decimals _`, `prime factors of 360 _`, `gcd of 48 and 180 _`, `stats of 3 5 8 13 _`, `5 choose 2 _`, `probability of 3 heads in 5 flips _`). `convert` / `calc` claim the `_` only when a number follows, so `convert this to markdown _` stays a rewrite. With a decision package, a plain phrasing (`what port does postgres use _`) reaches the same table through the `_` route |
