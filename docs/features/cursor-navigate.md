...
1. **Gate**: cursor-navigate only runs when `cursor-navigate: active` is set in `OPENCUES.md` frontmatter
...
In `OPENCUES.md` frontmatter (the cyclable values are auto-derived from `@opencues/core`'s `FEATURES` registry):
...
- The navigability criteria (cue-blank, LLM alts, spans) are the same as manual navigation
...
- Check navigability using the same sources as manual navigation (cue-blanks, dynamic defs, spans)
