...
3. **`FluidBlankSource` (priority 92)** — free-form lookup. A single fused LLM call segments the lookup span around `_` (handling ambient phrasing, embedded WH-questions, compact factual claims) and produces the canonical short answer. Fill is **additive by default**: only the `_` is replaced; the surrounding words are never overwritten. It replaces the **whole field** (WIPE) only when the host declares the field's shape — `AmbientContext.singleLine` (a search box / omnibox) with the buffer holding *nothing but* the query, or `AmbientContext.disposable` (a transient command field) — both gated by the `bufferIsExactlyTheLookup` floor so there is provably nothing but the query to remove. The model's `MODE` line is ignored; the runtime decides from the field declaration, never from a sentence-shape guess (the guess-based WIPE was retired in the July 2026 slim-down, commit `f62dcd28`, for buffer-safety; the field-declared successor is data-loss-free by construction). See [docs/architecture/blank-sources.md](../architecture/blank-sources.md). Always on — the base layer every unclaimed `_` falls through to (no mode scalar).
