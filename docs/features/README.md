...
| 39 | [BlankIntent](blank-intent.md) | LLM invocation gate for keyword script-blanks: prose that merely mentions a keyword CEDEs (`the weather was lovely today _` → nothing), real invocations INVOKE (`weather london _`). Keyword still required (consent); the LLM refines precision and extracts typed get/set/step — `volume 30 _` sets, `volume up _` steps. Line-scoped reach (keyword anywhere on the `_`'s line). Falls back to the proximity gate on error. OFF by default. |
