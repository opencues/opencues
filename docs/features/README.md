...
| 45 | [Replace Parse](replace-parse.md) | `replace-parse-mode` (off by default) — a parallel detector call beside TransformBlank's fused call catches single-substring edits ("her name is Sarha fix the spelling _"); a claim that survives the deterministic verbatim-substring gate splices boundedly instead of whole-buffer merging. Fallback on anything else is the fused merge, unchanged. |
