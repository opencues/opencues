...
| 38 | [Model Override](model-override.md) | Pin a specific LLM provider/model for ONE blank call without changing your default. Add `with <name>` anywhere in the buffer before `_` (`make formal X with opus _`, `atomic number of oxygen with cerebras _`) and the next fluid-blank lookup or transform-blank rewrite dispatches through that target. Per-call only — `~/.cues/OPENCUES.md` scalars stay untouched, the next `_` keystroke goes back to your configured bucket. Supports common aliases (opus / haiku / sonnet / nano / mini / flash / gpt-oss / llama / claude), provider ids, exact model names, and prefix / substring matches against any registered `knownModels`. Always on — no scalar to enable. |
