...
| 60 | [The decision layer](decisions.md) | `decisions-provider` (off by default) — a calibrated decision model under every place a chat call used to make a *decision*: tips, session contradiction, the ask gate, spelling, the sentence gate, the `_` router, replace-detect, settings / providers / undo, the command span, devices, tables, contradiction claims, calendar availability, community rules, the word gate. The model picks, code computes; text is generated only when text was asked for. The demo page: one line per behaviour. |
| 59 | [Tables & calculators](tables.md) | `table-lookups-mode` (off by default) — eight offline tables (unicode, colours, http, mime, ports, units, arithmetic, chemistry) and a calculator library (dates & durations, timezones, number formatting & number theory; percent/money, text, encodings/geometry planned) answer a `_` with no LLM, no network, and no way to be wrong the way a generated answer can. Keyword forms always; plain phrasings through the decision layer's `table` leg. |
...
| 45 | [Replace Parse](replace-parse.md) | `replace-parse-mode` (on by default) — a parallel detector call beside TransformBlank's fused call catches single-substring edits ("her name is Sarha fix the spelling _"); a claim that survives the deterministic verbatim-substring gate splices boundedly instead of whole-buffer merging. Fallback on anything else is the fused merge, unchanged. |
...
| 57 | [Glimmer Transition](glimmer-transition.md) | Scramble-settle arrival animation when a substitution LANDS (fluid-blank / transform-blank / keyword fills). One `glimmer-transition-ms` scalar (`off \| 300 \| 600 \| 900`, default 300). Display-only: buffer commits instantly, frames ride `textOverride` + `forceRender`. CC + Gemini paint it; other hosts keep the instant swap. |
...
| 55 | [AskUserQuestion Cues](ask-cues.md) | Generic tool-prompt cue primitive: borrow a well-known tool's system prompt (AskUserQuestion first) as a cue *generator* — the sentence under your cursor becomes an inline question (`❓` tip) with cyclable options (Ctrl+Alt+↑/↓); options carrying a concrete `apply` rewrite the sentence, advisory ones just inform. No new UI (reuses navigate + cycle + tip). Ambient at cursor, cached per sentence (one LLM call per new sentence). `TOOL_PROMPTS` registry — adding a tool is one entry. One `ask-cues-mode` scalar, OFF by default — benchmarked at ~1 useful question in 3 shown on realistic drafts, which is below the bar for a default (see `tests/benchmarks/ask-cues/EXPERIMENTS.md`). |
