...
**isSensitiveField** — The chrome bootstrap's gate that refuses attach + ambient + user-context on credential-adjacent fields. Three layers: input-type allow-list (`text` / `email` / `search` / `url` / `textarea`), autocomplete-token deny-list (`current-password`, `cc-number`, `one-time-code`, `autocomplete=off`, etc.), and name/id regex heuristic (`password|passwd|pwd|cvv|cvc|ssn|sin|pin|otp|secret|token|api[_-]?key|access[_-]?key|auth`). False positives accepted; never leak credentials.
