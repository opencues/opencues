...
**FluidBlankSource** — Free-form `_` lookup. Single FUSED LLM call emits both SPAN and ANSWER in one breath; the call also optionally receives sanitized ambient field metadata (when `ambient-context-mode: on`) so the segmenter can use the field's label as the question source for meta-triggers like `_` / `answer _`. Also optionally receives sentinels (when `sentinels-mode: safe` or `: raw`) — `~/.cues/SENTINELS.md` frontmatter as a catalog of sentinel tokens (`[FIRST NAME]`, `[EMAIL]`, etc.) so lookups personalise; a runtime post-processor substitutes real values after the LLM responds (safe mode keeps PII off the provider's logs). Handles math, factual, translation, unit conversion, codes, etc. without per-mode classification. Fires on `_` slots that no `BlankSource` or `TransformBlankSource` claimed. Opt-in via `fluid-blank-mode: on`.
...
**ValueSpec / exposeInMenu** — Each FEATURES entry's `values` array carries `{id, description, exposeInMenu?}` per value (not just a string list). `exposeInMenu: false` makes a value **parser-valid but absent from the cycling menu** — formal mechanism for hiding footgun modes (today's only user: `sentinels-mode: raw`, which inlines PII into LLM prompts and shouldn't be flipped by a keystroke). Replaces the old "hidden by absence from OPENCUES.md `settings:` block" pattern.
