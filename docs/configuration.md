...
In `safe` mode (recommended), the LLM only sees a catalog of token names + descriptions (`[EMAIL] — user's email`); it emits sentinels like `[EMAIL]`, and a runtime post-processor swaps in the real value AFTER the response — your PII never reaches the provider's logs.
